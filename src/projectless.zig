...
//! Own `next_daemon_key` band. Cancel in-flight zeros the key so a
//! cancelled sidecar cannot paint a later session. Local
//! `sessions.json` stays canonical. Best-effort sidecar only.
//! Leftover: `migrateProjectlessWorkspace`; reusing an unstarted
//! projectless draft (Waku `create_projectless_session` selects the
//! draft) is skipped this cut so New Task still always creates a
//! row.
