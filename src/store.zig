...
//! restore); `reload_token`, Hard Reload's pending blank hop, and Stop loading's loading-guess / blank hop stay runtime-only; occupied Terminal slots persist as
...
//! directory expands / Browser occupancy / Terminal occupancy); session-switch panel
//! open/closed, nested Files-tree / Diff list widths, Browser
...
//! `opencode2_server_password` / `opencode2_server_username` /
...
pub fn saveSession(model: *const Model, session_id: u32, allocator: std.mem.Allocator, io: std.Io) !void {
    ...
    document.opencode2_server_username = model.opencode2ServerUsername();
...
const Document = struct {
    ...
    opencode2_server_username: []const u8 = "",
    ...
    fn empty(model: *const Model) Document {
        return .{
            ...
            .opencode2_server_username = model.opencode2ServerUsername(),
...
fn applyCatalog(model: *Model, allocator: std.mem.Allocator, bytes: []const u8) !void {
    ...
    model.setOpencode2ServerUsername(document.opencode2_server_username);
...
fn parseDocument(arena: std.mem.Allocator, bytes: []const u8) !Document {
    ...
    const parsed_ui_font_size = parseUiFontSize(obj.get("ui_font_size"));
    return .{
        .version = version,
        ...
        .opencode2_server_username = parseOpencode2ServerUsername(obj.get("opencode2_server_username")),
...
fn parseSession(arena: std.mem.Allocator, value: std.json.Value) !StoredSession {
    ...
    const parsed_workspace = parseWorkspace(obj.get("workspace"));
...
fn parseOpencode2ServerUsername(value: ?std.json.Value) []const u8 {
    const raw = jsonString(value) orelse return "";
    const trimmed = std.mem.trim(u8, raw, " \t\r\n");
    if (trimmed.len == 0 or trimmed.len > model_exports.max_opencode2_server_username) return "";
    return trimmed;
}
...
/// Missing / unknown / out-of-range `code_font_size` copies sanitized
/// `ui_font_size` so a #549 UI bump keeps larger message/code body.
/// Explicit valid ladder value wins.
fn parseCodeFontSize(value: ?std.json.Value, ui_font_size: u8) u8 {
    const n = jsonUint(value) orelse return ui_font_size;
    for (model_exports.code_font_sizes) |allowed| {
...
fn encodeDocument(allocator: std.mem.Allocator, document: Document) ![]u8 {
    ...
    try out.appendSlice(allocator, ",\"code_font_size\":");
    ...
    try out.appendSlice(allocator, if (document.file_preview_find_whole_word) "true" else "false");
    try out.appendSlice(allocator, ",\"file_preview_find_use_regex\":");
    try out.appendSlice(allocator, if (document.file_preview_find_use_regex) "true" else "false");
    ...
    try out.appendSlice(allocator, ",\"opencode2_server_username\":");
    try appendJsonString(&out, allocator, document.opencode2_server_username);
...
fn appendSession(out: *std.ArrayList(u8), allocator: std.mem.Allocator, session: StoredSession) !void {
    ...
    switch (session.workspace_kind) {
        ...
        .worktree => {
            ...
            try appendJsonString(out, allocator, session.workspace_branch);
            try out.append(allocator, '}');
        },
    }
...
test "browser_histories round-trips rings and index; legacy tip-only stays single-entry" {
    ...
    try testing.expect(std.mem.indexOf(u8, bytes, "\"loading_guess_until_ms\"") == null);
    try testing.expect(std.mem.indexOf(u8, bytes, "\"stop_blank_pending\"") == null);
...
test "settings extras persist last_model access path and daemon; missing catalog is not created" {
    ...
    try testing.expectEqual(@as(u8, 18), loaded.code_font_size);
    ...
    try testing.expectEqual(@as(u8, 18), cleared.code_font_size);
...
test "ui_font_size missing or unknown or out-of-range loads as 14; extras roundtrip" {
    ...
    try testing.expectEqual(model_exports.default_code_font_size, missing.code_font_size);
...
test "code_font_size missing or unknown or out-of-range copies sanitized ui_font_size; explicit valid wins" {
    const testing = std.testing;
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var dir_buf: [256]u8 = undefined;
    ...
    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"ui_font_size":20,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var missing = Model{};
    missing.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&missing, allocator, io));
    try testing.expectEqual(@as(u8, 20), missing.ui_font_size);
    try testing.expectEqual(@as(u8, 20), missing.code_font_size);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"ui_font_size":16,"code_font_size":"nope","sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var unknown = Model{};
    unknown.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&unknown, allocator, io));
    try testing.expectEqual(@as(u8, 16), unknown.ui_font_size);
    try testing.expectEqual(@as(u8, 16), unknown.code_font_size);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"ui_font_size":18,"code_font_size":17,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var out_of_range = Model{};
    out_of_range.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&out_of_range, allocator, io));
    try testing.expectEqual(@as(u8, 18), out_of_range.ui_font_size);
    try testing.expectEqual(@as(u8, 18), out_of_range.code_font_size);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var both_missing = Model{};
    both_missing.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&both_missing, allocator, io));
    try testing.expectEqual(model_exports.default_ui_font_size, both_missing.ui_font_size);
    try testing.expectEqual(model_exports.default_code_font_size, both_missing.code_font_size);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"ui_font_size":20,"code_font_size":11,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var explicit = Model{};
    explicit.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&explicit, allocator, io));
    try testing.expectEqual(@as(u8, 20), explicit.ui_font_size);
    try testing.expectEqual(@as(u8, 11), explicit.code_font_size);

    var source = Model{};
    source.task_state_loaded = true;
    source.setStoreDir(dir);
    source.store_io = io;
    const id = source.addSession("code font later", .fx);
    _ = source.appendTurn(id, .user, "remember code font");
    try saveSession(&source, id, allocator, io);

    source.setUiFontSize(11);
    source.setCodeFontSize(20);
    persistSettingsIfPossible(&source);
    var split = Model{};
    split.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&split, allocator, io));
    try testing.expectEqual(@as(u8, 11), split.ui_font_size);
    try testing.expectEqual(@as(u8, 20), split.code_font_size);

    source.setCodeFontSize(17);
    persistSettingsIfPossible(&source);
    var sanitized = Model{};
    sanitized.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&sanitized, allocator, io));
    try testing.expectEqual(model_exports.default_code_font_size, sanitized.code_font_size);
    try testing.expectEqual(@as(u8, 11), sanitized.ui_font_size);
}
...
test "opencode2_server_username extras persist round-trip; empty/missing/overflow stay empty" {
    const testing = std.testing;
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var dir_buf: [256]u8 = undefined;
    ...
    missing.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&missing, allocator, io));
    try testing.expectEqualStrings("", missing.opencode2ServerUsername());

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"opencode2_server_username":"   ","sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var blank = Model{};
    blank.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&blank, allocator, io));
    try testing.expectEqualStrings("", blank.opencode2ServerUsername());

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"opencode2_server_username":null,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var nulls = Model{};
    nulls.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&nulls, allocator, io));
    try testing.expectEqualStrings("", nulls.opencode2ServerUsername());

    const overflow = "u" ** 257;
    var overflow_json: std.ArrayList(u8) = .empty;
    defer overflow_json.deinit(allocator);
    try overflow_json.appendSlice(allocator, "{\"version\":1,\"selected\":1,\"next_id\":2,\"next_turn_id\":2,\"next_queued_id\":1,\"opencode2_server_username\":\"");
    try overflow_json.appendSlice(allocator, overflow);
    try overflow_json.appendSlice(allocator, "\",\"sessions\":[{\"id\":1,\"title\":\"legacy\",\"provider\":\"fx\",\"untitled\":false,\"has_started\":true,\"turns\":[{\"id\":1,\"role\":\"user\",\"body\":\"hi\"}],\"queued_messages\":[]}]}");
    try writeRaw(io, dir, overflow_json.items);
    var too_long = Model{};
    too_long.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&too_long, allocator, io));
    try testing.expectEqualStrings("", too_long.opencode2ServerUsername());

    var source = Model{};
    source.task_state_loaded = true;
    source.setStoreDir(dir);
    source.store_io = io;
    const id = source.addSession("username later", .opencode2);
    _ = source.appendTurn(id, .user, "remember username");
    try saveSession(&source, id, allocator, io);
    source.setOpencode2ServerUsername("  alice  ");
    persistSettingsIfPossible(&source);
    try saveSession(&source, id, allocator, io);
    ...
    loaded.store_io = io;
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&loaded, allocator, io));
    try testing.expectEqualStrings("alice", loaded.opencode2ServerUsername());

    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    const bytes = try std.Io.Dir.cwd().readFileAlloc(io, catalogPath(dir, &path_buf).?, allocator, .limited(64 * 1024));
    defer allocator.free(bytes);
    try testing.expect(std.mem.indexOf(u8, bytes, "\"opencode2_server_username\":\"alice\"") != null);

    loaded.setOpencode2ServerUsername("");
    persistSettingsIfPossible(&loaded);
    var cleared = Model{};
    cleared.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&cleared, allocator, io));
    try testing.expectEqualStrings("", cleared.opencode2ServerUsername());
    const cleared_bytes = try std.Io.Dir.cwd().readFileAlloc(io, catalogPath(dir, &path_buf).?, allocator, .limited(64 * 1024));
    defer allocator.free(cleared_bytes);
    try testing.expect(std.mem.indexOf(u8, cleared_bytes, "\"opencode2_server_username\":\"\"") != null);

    var overflow_set = Model{};
    overflow_set.setOpencode2ServerUsername(overflow);
    try testing.expectEqualStrings("", overflow_set.opencode2ServerUsername());
}
...
test "render_math extras persist on sessions.json; missing or unknown load true" {
    const testing = std.testing;
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var dir_buf: [256]u8 = undefined;
    ...
    missing_catalog.setStoreDir(dir);
    missing_catalog.store_io = io;
    missing_catalog.render_math = false;
    persistSettingsIfPossible(&missing_catalog);
    var missing_path: [std.fs.max_path_bytes]u8 = undefined;
    try testing.expectError(error.FileNotFound, std.Io.Dir.cwd().readFileAlloc(io, catalogPath(dir, &missing_path).?, allocator, .limited(64)));
    ...
    missing.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&missing, allocator, io));
    try testing.expect(missing.render_math);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"render_math":null,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var nulls = Model{};
    nulls.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&nulls, allocator, io));
    try testing.expect(nulls.render_math);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"render_math":"yes","sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var unknown = Model{};
    unknown.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&unknown, allocator, io));
    try testing.expect(unknown.render_math);

    var source = Model{};
    source.task_state_loaded = true;
    source.setStoreDir(dir);
    source.store_io = io;
    const id = source.addSession("render math later", .fx);
    _ = source.appendTurn(id, .user, "remember render math chrome");
    try saveSession(&source, id, allocator, io);

    var defaulted = Model{};
    defaulted.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&defaulted, allocator, io));
    try testing.expect(defaulted.render_math);

    source.render_math = false;
    persistSettingsIfPossible(&source);
    var off = Model{};
    off.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&off, allocator, io));
    try testing.expect(!off.render_math);

    source.render_math = true;
    persistSettingsIfPossible(&source);
    var on = Model{};
    on.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&on, allocator, io));
    try testing.expect(on.render_math);
...
test "file_preview_find option extras persist on sessions.json; missing or unknown load false" {
    const testing = std.testing;
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var dir_buf: [256]u8 = undefined;
    ...
    missing_catalog.setStoreDir(dir);
    missing_catalog.store_io = io;
    missing_catalog.file_preview_find_case_sensitive = true;
    missing_catalog.file_preview_find_whole_word = true;
    missing_catalog.file_preview_find_use_regex = true;
    persistSettingsIfPossible(&missing_catalog);
    var missing_path: [std.fs.max_path_bytes]u8 = undefined;
    try testing.expectError(error.FileNotFound, std.Io.Dir.cwd().readFileAlloc(io, catalogPath(dir, &missing_path).?, allocator, .limited(64)));
    ...
    missing.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&missing, allocator, io));
    try testing.expect(!missing.file_preview_find_case_sensitive);
    try testing.expect(!missing.file_preview_find_whole_word);
    try testing.expect(!missing.file_preview_find_use_regex);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"file_preview_find_case_sensitive":null,"file_preview_find_whole_word":null,"file_preview_find_use_regex":null,"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var nulls = Model{};
    nulls.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&nulls, allocator, io));
    try testing.expect(!nulls.file_preview_find_case_sensitive);
    try testing.expect(!nulls.file_preview_find_whole_word);
    try testing.expect(!nulls.file_preview_find_use_regex);

    try writeRaw(io, dir,
        \\{"version":1,"selected":1,"next_id":2,"next_turn_id":2,"next_queued_id":1,"file_preview_find_case_sensitive":"yes","file_preview_find_whole_word":1,"file_preview_find_use_regex":[],"sessions":[{"id":1,"title":"legacy","provider":"fx","untitled":false,"has_started":true,"turns":[{"id":1,"role":"user","body":"hi"}],"queued_messages":[]}]}
    );
    var unknown = Model{};
    unknown.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&unknown, allocator, io));
    try testing.expect(!unknown.file_preview_find_case_sensitive);
    try testing.expect(!unknown.file_preview_find_whole_word);
    try testing.expect(!unknown.file_preview_find_use_regex);

    var source = Model{};
    source.task_state_loaded = true;
    source.setStoreDir(dir);
    source.store_io = io;
    const id = source.addSession("find options later", .fx);
    _ = source.appendTurn(id, .user, "remember find options");
    try saveSession(&source, id, allocator, io);

    source.file_preview_find_case_sensitive = true;
    source.file_preview_find_whole_word = true;
    source.file_preview_find_use_regex = true;
    persistSettingsIfPossible(&source);
    var loaded = Model{};
    loaded.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&loaded, allocator, io));
    try testing.expect(loaded.file_preview_find_case_sensitive);
    try testing.expect(loaded.file_preview_find_whole_word);
    try testing.expect(loaded.file_preview_find_use_regex);

    source.file_preview_find_case_sensitive = true;
    source.file_preview_find_whole_word = false;
    source.file_preview_find_use_regex = true;
    persistSettingsIfPossible(&source);
    var mixed = Model{};
    mixed.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&mixed, allocator, io));
    try testing.expect(mixed.file_preview_find_case_sensitive);
    try testing.expect(!mixed.file_preview_find_whole_word);
    try testing.expect(mixed.file_preview_find_use_regex);

    source.file_preview_find_case_sensitive = false;
    source.file_preview_find_whole_word = false;
    source.file_preview_find_use_regex = false;
    persistSettingsIfPossible(&source);
    var closed = Model{};
    closed.setStoreDir(dir);
    try testing.expectEqual(LoadKind.loaded, loadCatalog(&closed, allocator, io));
    try testing.expect(!closed.file_preview_find_case_sensitive);
    try testing.expect(!closed.file_preview_find_whole_word);
    try testing.expect(!closed.file_preview_find_use_regex);
