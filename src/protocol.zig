...
//! 5s refresh tick ships; first-cut 1s elapsed duration labels
//! ship. Not full BackgroundWorkRegistry.
//!
...
//! `{ "type": "inspectBranches", "cwd": "<path>" }` (`cwd` stays cwd), and
...
//! JSON string via `writeJsonString`), and
//! `WorkspaceOperation::CopySessionRefs`
//! `{ "type": "copySessionRefs", "cwd", "source_session_id",
//! "target_session_id", "through_turn_count" }` (camelCase op tag;
//! snake_case session/turn fields. WorkspaceOperation serde is
//! `rename_all = "camelCase"` on the enum tag only — no
//! `rename_all_fields`. Live Waku `apps/web/src/lib/daemon-api.ts`
...
//! DeleteSessionRefs, not camelCase `gitRef`), and
//! `WorkspaceOperation::CaptureRef`
//! `{ "type": "captureRef", "cwd", "git_ref" }` (camelCase op tag;
//! snake_case `git_ref` like HasRef, not camelCase `gitRef`), and
//! `WorkspaceOperation::RestoreRef`
//! `{ "type": "restoreRef", "cwd", "git_ref" }` (camelCase op tag;
//! snake_case `git_ref` like CaptureRef / HasRef, not camelCase
//! `gitRef`; no force and no amend), and
...
//! `WorkspaceOperation::MigrateProjectlessWorkspace`
//! `{ "type": "migrateProjectlessWorkspace", "path": "/abs/legacy/path" }`
//! (camelCase op tag; field is plain `path`, always present. Same
//! serde as CreateProjectlessWorkspace: `rename_all` is the camelCase
//! `type` tag only — no `rename_all_fields`. No force/amend/cwd/
//! prompt/project_path). Ok is the same nested
//! `WorkspaceResult::ProjectlessWorkspace` as Create — wire
//! `result: { "type": "projectlessWorkspace", "cwd": "/abs/path" }`
//! under `outcome.payload` (not Ack, not Bool, not a bare
//! projectlessWorkspace object). Empty `cwd` is rejected. Nil
//! request-frame `sessionId` / `runtimeId`.
...
//! "branch" } }` (not a bare ack) — InspectBranches yields
...
//! still ok) — and CreateProjectlessWorkspace / MigrateProjectlessWorkspace yield
...
/// `ListProjectFiles`, `DiscoverSlashCommands`,
/// `CreateProjectlessWorkspace`, and `MigrateProjectlessWorkspace`
...
/// and `CopySessionRefs { cwd, source_session_id, target_session_id,
/// through_turn_count }` (snake_case session/turn fields on the
/// wire — WorkspaceOperation has serde `rename_all = "camelCase"`
/// on the enum tag only, not `rename_all_fields`; live Waku
/// CaptureTurnStart / SessionTurnRefs send `session_id` /
...
/// `CaptureRef { cwd, git_ref }` (snake_case `git_ref`, not
...
pub const WorkspaceCheckoutBranch = struct {
    cwd: []const u8,
    branch: []const u8,
    create: bool,
};
...
/// Waku `MigrateProjectlessWorkspace`. Wire key `path` is a JSON
/// string and is always present (WorkspaceOperation serde
/// `rename_all` is the camelCase `type` tag only). No force, no
/// amend, no cwd/prompt/project_path. Ok is nested
/// `WorkspaceResult::ProjectlessWorkspace` (same as Create).
pub const WorkspaceMigrateProjectlessWorkspace = struct {
    path: []const u8,
};
...
/// Waku `CopySessionRefs`. Wire keys stay snake_case
/// `source_session_id` / `target_session_id` / `through_turn_count`
/// (WorkspaceOperation serde `rename_all` is the camelCase `type`
/// tag only). `through_turn_count` is Waku's 1-based prompt
/// ordinal; the daemon copies `0..=through_turn_count`.
pub const WorkspaceCopySessionRefs = struct {
    cwd: []const u8,
...
/// Waku `CaptureRef`. Wire keys stay snake_case `git_ref`
/// (WorkspaceOperation serde `rename_all` is the camelCase `type`
/// tag only). Same snake_case field as HasRef, not camelCase
/// `gitRef`. Ok is workspace Ack.
pub const WorkspaceCaptureRef = struct {
    cwd: []const u8,
    git_ref: []const u8,
};

/// Waku `RestoreRef`. Wire keys stay snake_case `git_ref`
/// (WorkspaceOperation serde `rename_all` is the camelCase `type`
/// tag only). Same snake_case field as CaptureRef / HasRef, not
/// camelCase `gitRef`. No force and no amend. Ok is workspace Ack
/// (nested `result: { "type": "ack" }`, not Bool, not a bare ack).
pub const WorkspaceRestoreRef = struct {
    cwd: []const u8,
    git_ref: []const u8,
};

/// Waku `DeleteRef`. Wire keys stay snake_case `git_ref`
/// (WorkspaceOperation serde `rename_all` is the camelCase `type`
/// tag only). Same snake_case field as RestoreRef / CaptureRef /
/// HasRef, not camelCase `gitRef`. No force and no amend. Ok is
/// workspace Ack (nested `result: { "type": "ack" }`, not Bool,
/// not a bare ack).
pub const WorkspaceDeleteRef = struct {
    cwd: []const u8,
    git_ref: []const u8,
};
...
pub const WorkspaceOperation = union(WorkspaceKind) {
    ...
    migrate_projectless_workspace: WorkspaceMigrateProjectlessWorkspace,
};

/// Local heads from an ok `branches` or `branchChanged` workspace
/// result. Waku lists `refs/heads` only (no remotes). Slices alias
/// the JSON arena.
...
/// Slices alias the JSON arena used to parse the line. Empty `cwd`
/// does not set `ok`.
...
/// GenerateCommitMessage, ListTree, CollectReviewDiff,
/// BrowseDirectory, ReadTextFile, WriteTextFile,
/// CopySessionRefs, DeleteSessionRefs, HasRef, CaptureRef, RestoreRef, DeleteRef, DeleteTurnRefsAfter, SessionTurnRefs, ListProjectFiles, DiscoverSlashCommands, CreateProjectlessWorkspace, and MigrateProjectlessWorkspace pass `NIL_UUID` for those ids.
...
fn writeWorkspaceOperation(cur: *Cursor, operation: WorkspaceOperation) WriteError!void {
    switch (operation) {
        ...
        .checkout_branch => |args| {
            try cur.write("{\"type\":\"checkoutBranch\",\"cwd\":");
            try writeJsonString(cur, args.cwd);
            try cur.write(",\"branch\":");
        ...
        .copy_session_refs => |args| {
            try cur.write("{\"type\":\"copySessionRefs\",\"cwd\":");
            try writeJsonString(cur, args.cwd);
            try cur.write(",\"source_session_id\":");
            try writeJsonString(cur, args.source_session_id);
            try cur.write(",\"target_session_id\":");
        ...
        .capture_ref => |args| {
            try cur.write("{\"type\":\"captureRef\",\"cwd\":");
            try writeJsonString(cur, args.cwd);
            try cur.write(",\"git_ref\":");
            try writeJsonString(cur, args.git_ref);
            try cur.write("}");
        ...
        .restore_ref => |args| {
            try cur.write("{\"type\":\"restoreRef\",\"cwd\":");
            try writeJsonString(cur, args.cwd);
            try cur.write(",\"git_ref\":");
            try writeJsonString(cur, args.git_ref);
            try cur.write("}");
        ...
        .delete_ref => |args| {
            try cur.write("{\"type\":\"deleteRef\",\"cwd\":");
            try writeJsonString(cur, args.cwd);
            try cur.write(",\"git_ref\":");
            try writeJsonString(cur, args.git_ref);
            try cur.write("}");
        ...
        .migrate_projectless_workspace => |args| {
            try cur.write("{\"type\":\"migrateProjectlessWorkspace\",\"path\":");
            try writeJsonString(cur, args.path);
            try cur.write("}");
        },
    }
}
...
pub fn parseBranches(allocator: std.mem.Allocator, line: []const u8) ParsedBranches {
    return parseWorkspaceSnapshot(allocator, line, "branches");
}

/// Light parser for ok CheckoutBranch. True/`ok` when an ok `response`
/// carries nested `result: { "type": "branchChanged", "snapshot": { … } }`
/// with the same snake_case `BranchSnapshot` shape InspectBranches
/// parses. Null snapshot, missing fields, camelCase occupancy, or an
/// empty `branches` array are rejected. Slices alias `allocator`.
pub fn parseBranchChanged(allocator: std.mem.Allocator, line: []const u8) ParsedBranches {
    return parseWorkspaceSnapshot(allocator, line, "branchChanged");
}

fn parseWorkspaceSnapshot(allocator: std.mem.Allocator, line: []const u8, result_type: []const u8) ParsedBranches {
    var parsed = ParsedBranches{};
    const result = workspaceResultObject(allocator, line) orelse return parsed;
    if (!std.mem.eql(u8, jsonStringValue(result.get("type")) orelse "", result_type)) return parsed;
...
/// Light parser for ok CreateProjectlessWorkspace /
/// MigrateProjectlessWorkspace. True/`ok` when an ok `response`
/// carries nested
/// `result: { "type": "projectlessWorkspace", "cwd": "/abs/path" }`
...
/// MigrateProjectlessWorkspace `projectlessWorkspace` with a
/// non-empty `cwd`, or
...
test "workspace request wraps camelCase checkoutBranch with cwd, branch, create, and nil ids" {
    var buf: [512]u8 = undefined;
    const checkout = try writeWorkspace(
        &buf,
        "00000000-0000-0000-0000-000000000014",
        NIL_UUID,
        NIL_UUID,
        .{ .checkout_branch = .{ .cwd = "/tmp/faku", .branch = "feat", .create = false } },
    );
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"type\":\"request\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"requestId\":\"00000000-0000-0000-0000-000000000014\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"checkoutBranch\",\"cwd\":\"/tmp/faku\",\"branch\":\"feat\",\"create\":false}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"create\":true") == null);
    try std.testing.expect(std.mem.indexOf(u8, checkout, "\"type\":\"inspectBranches\"") == null);
    ...
    const created = try writeWorkspace(
        ...
        NIL_UUID,
        NIL_UUID,
        .{ .checkout_branch = .{ .cwd = "/tmp/faku", .branch = "feat/new", .create = true } },
    );
    try std.testing.expect(std.mem.indexOf(u8, created, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"checkoutBranch\",\"cwd\":\"/tmp/faku\",\"branch\":\"feat/new\",\"create\":true}}") != null);
...
test "parseBranchChanged extracts snake_case snapshot and rejects branches, ack, or error" {
    const allocator = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    const ok_line = "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"branchChanged\",\"snapshot\":{\"repository\":\"/tmp/faku\",\"current\":\"feat\",\"detached_head\":null,\"default_branch\":\"main\",\"branches\":[{\"name\":\"feat\",\"checked_out_elsewhere\":false},{\"name\":\"main\",\"checked_out_elsewhere\":false}],\"additions\":0,\"deletions\":0}}}}}";
    const parsed = parseBranchChanged(arena, ok_line);
    try std.testing.expect(parsed.ok);
    try std.testing.expectEqualStrings("feat", parsed.current);
    try std.testing.expectEqualStrings("", parsed.detached_head);
    try std.testing.expectEqualStrings("main", parsed.default_branch);
    try std.testing.expectEqual(@as(usize, 2), parsed.entry_count);
    try std.testing.expectEqualStrings("feat", parsed.entries[0].name);
    try std.testing.expect(isWorkspaceSuccess(arena, ok_line));
    try std.testing.expect(!isWorkspaceAck(arena, ok_line));
    try std.testing.expect(!parseBranches(arena, ok_line).ok);
    try std.testing.expect(!parseWorktreeCreated(arena, ok_line).ok);

    const detached_line = "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"branchChanged\",\"snapshot\":{\"repository\":\"/tmp/faku\",\"current\":null,\"detached_head\":\"a1b2c3d\",\"default_branch\":null,\"branches\":[{\"name\":\"main\",\"checked_out_elsewhere\":false}],\"additions\":0,\"deletions\":0}}}}}";
    const detached = parseBranchChanged(arena, detached_line);
    try std.testing.expect(detached.ok);
    try std.testing.expectEqualStrings("", detached.current);
    try std.testing.expectEqualStrings("a1b2c3d", detached.detached_head);

    try std.testing.expect(!parseBranchChanged(arena, "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"branchChanged\",\"snapshot\":null}}}}").ok);
    try std.testing.expect(!isWorkspaceSuccess(arena, "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"branchChanged\",\"snapshot\":null}}}}"));
...
test "workspace request wraps camelCase copySessionRefs with snake_case session/turn fields and nil ids" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"copySessionRefs\",\"cwd\":\"/tmp/faku\",\"source_session_id\":\"00000000-0000-0000-0000-000000000007\",\"target_session_id\":\"00000000-0000-0000-0000-000000000008\",\"through_turn_count\":2}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"through_turn_count\":2") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"through_turn_count\":\"2\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "sourceSessionId") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "targetSessionId") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "throughTurnCount") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"captureTurnStart\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"captureTurn\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"writeTextFile\"") == null);
...
test "workspace request wraps camelCase captureRef with snake_case git_ref and nil ids" {
    var buf: [768]u8 = undefined;
    const json = try writeWorkspace(
        &buf,
        "00000000-0000-0000-0000-000000000014",
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"captureRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"hasRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"deleteSessionRefs\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"copySessionRefs\"") == null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "force") == null);
    {
        ...
        defer arena_state.deinit();
        try std.testing.expect(isWorkspaceAck(arena_state.allocator(), "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"ack\"}}}}"));
        try std.testing.expect(!parseWorkspaceBool(arena_state.allocator(), "{\"type\":\"response\",\"requestId\":\"00000000-0000-0000-0000-000000000014\",\"outcome\":{\"status\":\"ok\",\"payload\":{\"type\":\"workspace\",\"result\":{\"type\":\"ack\"}}}}").ok);
    }
...
test "workspace request wraps camelCase restoreRef with snake_case git_ref and nil ids" {
    var buf: [768]u8 = undefined;
    const json = try writeWorkspace(
        &buf,
        "00000000-0000-0000-0000-000000000014",
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"restoreRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"captureRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"hasRef\"") == null);
...
test "workspace request wraps camelCase deleteRef with snake_case git_ref and nil ids" {
    var buf: [768]u8 = undefined;
    const json = try writeWorkspace(
        &buf,
        "00000000-0000-0000-0000-000000000014",
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"deleteRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"restoreRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, json, "\"type\":\"captureRef\"") == null);
...
test "workspace request wraps camelCase migrateProjectlessWorkspace with path and nil ids" {
    var buf: [1024]u8 = undefined;
    const encoded = try writeWorkspace(
        &buf,
        "00000000-0000-0000-0000-000000000014",
        NIL_UUID,
        NIL_UUID,
        .{ .migrate_projectless_workspace = .{ .path = "/home/me/.waku/2026-09-06/new-chat" } },
    );
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"type\":\"request\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"requestId\":\"00000000-0000-0000-0000-000000000014\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"sessionId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"runtimeId\":\"" ++ NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"migrateProjectlessWorkspace\",\"path\":\"/home/me/.waku/2026-09-06/new-chat\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"type\":\"createProjectlessWorkspace\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"type\":\"prompt\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"type\":\"attachSession\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"kind\":\"migrateProjectlessWorkspace\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "project_path") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "\"prompt\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "amend") == null);
    try std.testing.expect(std.mem.indexOf(u8, encoded, "force") == null);
