...
//! `--mode rpc`; Claude ACP / `--continue` / `--resume`.
//! Claude print-mode stream-json (image path in the `-p` prompt when
//! attached),
...
pub const claude_transport_note = "Live Send is one-shot claude -p --output-format stream-json when Available (image path in the -p prompt when attached).";
