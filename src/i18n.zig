...
//! evolvable),
//! plus first-cut Review Diff unmodified-line gap labels
//! (`{d} unmodified line` / `{d} unmodified lines`; same
...
//! Transcript / Conversation history / Review files /
//! Background work / Background output / Browser
...
//! Stop loading / Navigate and Secure / Not secure a11y (same
//! `BrowserToolbarChrome` strings; Stop loading is distinct from
//! `ComposerSendStopChrome` / `BackgroundChrome.daemon_stop`)
...
//! plus Review Diff unmodified-line gap labels (same
//! `ReviewDiffGapLabelChrome` strings; distinct from
//! `ReviewDiffChrome` / `ReviewHunkA11yChrome` so gap count
//! labels stay independently evolvable; numbers stay Latin)
...
//! OPENCODE_SERVER_PASSWORD UI ships (`ProvidersOpencodePasswordChrome`;
...
//! stays `apply_opencode2_password` / `clear_opencode2_password`;
//! typed password stays data)
...
//! expanded row only (same `show_opencode_attach` gate); on-press
//! stays `apply_opencode2_username` / `clear_opencode2_username`;
//! typed username stays data)
...
//! File preview source / Transcript / Conversation history /
//! Review files / Background work / Background output / Browser
//! follows the resolved locale this cut
//! (same `StructuralRegionChrome` strings; distinct from
//! `FilePreviewChrome` / `FilePreviewFindToggleChrome` /
//! `FilePreviewErrorChrome` / `FilePreviewFindMatchChrome` /
//! `TerminalRestartChrome` / `BrowserToolbarChrome` /
...
//! `browser_hard_reload` / `browser_stop_loading` / `browser_navigate`). Browser start-page globe icon a11y follows
...
//! stays independently evolvable). Review Diff unmodified-line
//! gap labels follow the resolved locale this cut (same
//! `ReviewDiffGapLabelChrome` strings; distinct from
//! `ReviewDiffChrome` / `ReviewHunkA11yChrome` so gap count
//! labels stay independently evolvable; numbers stay Latin).
...
const appearance_font_chrome_en: AppearanceFontChrome = .{
    .title = "UI font size",
    .description = "Text size across chrome and controls",
};

const appearance_font_chrome_zh_cn: AppearanceFontChrome = .{
    .title = "界面字号",
    .description = "界面与控件的文字大小",
};

const appearance_font_chrome_ja: AppearanceFontChrome = .{
    .title = "UI のフォントサイズ",
    .description = "インターフェースとコントロールの文字サイズ",
};

/// Settings → Appearance code font size title + description for the
/// resolved locale. Same resolve path as Chrome. Distinct from UI font
/// (`i18n.AppearanceFontChrome`) so this pack stays independently
/// evolvable. Chip labels stay Latin digits in every locale. Wire ids /
/// on-press stay English (`settings_code_font_11` … `settings_code_font_20`).
/// Native couples messages, code, diffs, and terminal to `body_size`.
pub const AppearanceCodeFontChrome = struct {
    title: []const u8,
    description: []const u8,
};

const appearance_code_font_chrome_en: AppearanceCodeFontChrome = .{
    .title = "Code font size",
    .description = "Text size for messages, code, diffs, and terminal",
};

const appearance_code_font_chrome_zh_cn: AppearanceCodeFontChrome = .{
    .title = "代码字号",
    .description = "消息、代码、差异和终端的文字大小",
};

const appearance_code_font_chrome_ja: AppearanceCodeFontChrome = .{
    .title = "コードのフォントサイズ",
    .description = "メッセージ、コード、差分、ターミナルの文字サイズ",
};
...
/// chrome live in `BackgroundChrome`. Pane Native `label=` a11y
/// (Review files / Browser / Background work / Background output /
/// Terminal) lives in `StructuralRegionChrome`, distinct from these
/// tab chips.
...
/// Review Diff unmodified-line gap labels for the resolved locale.
/// Same resolve path as ReviewHunkA11yChrome. English matches the
/// former hardcoded copy (`{d} unmodified line` / `{d} unmodified
/// lines`). Distinct from `ReviewDiffChrome` (title / Cancel /
/// source chips / gap Start|End|Both|All) and
...
const review_diff_gap_label_chrome_en: ReviewDiffGapLabelChrome = .{
    .unmodified_line = "{d} unmodified line",
    .unmodified_lines = "{d} unmodified lines",
};

const review_diff_gap_label_chrome_zh_cn: ReviewDiffGapLabelChrome = .{
    .unmodified_line = "{d} 行未修改",
    .unmodified_lines = "{d} 行未修改",
};
...
/// File preview source / Transcript / Conversation history /
/// Review files / Background work / Background output / Browser).
/// Same resolve path as ComposerRegionChrome.
/// English matches the former hardcoded copy. Distinct from
...
/// (visible Files/Review/Browser/Terminal/Background tab chips),
/// and `ComposerRegionChrome` so those packs stay independently
/// evolvable. Native surfaces `label=` as the accessible name.
pub const StructuralRegionChrome = struct {
    ...
    review_files: []const u8,
    background_work: []const u8,
    background_output: []const u8,
    browser: []const u8,
};
...
const structural_region_chrome_ja: StructuralRegionChrome = .{
    ...
    .review_files = "レビューファイル",
    .background_work = "バックグラウンド作業",
    .background_output = "バックグラウンド出力",
    .browser = "ブラウザ",
};
...
/// Browser toolbar Back / Forward / Reload / Hard Reload / Stop loading /
/// Navigate and Secure / Not secure a11y for the resolved locale. Same resolve
/// path as BrowserAddressChrome. English matches the former
/// hardcoded copy (Hard Reload / Stop loading are first-cut EN). Wire ids / on-press
/// stay English (`browser_back` / `browser_forward` /
/// `browser_reload` / `browser_hard_reload` / `browser_stop_loading` /
/// `browser_navigate`).
...
/// `history_back` / `history_forward`) and from composer Send/Stop
/// (`ComposerSendStopChrome`) / Background daemon Stop
/// (`BackgroundChrome.daemon_stop`).
pub const BrowserToolbarChrome = struct {
    back: []const u8,
    forward: []const u8,
    reload: []const u8,
    hard_reload: []const u8,
    stop_loading: []const u8,
    navigate: []const u8,
    secure: []const u8,
    not_secure: []const u8,
};
...
/// Runtime-only status (not `sessions.json`). Password UI ships
/// (`ProvidersOpencodePasswordChrome`; Check serve `-u
...
/// Settings Providers OpenCode 2 Serve password for the resolved
/// locale. Same resolve path as ProvidersChrome. Distinct from
/// `ProvidersOpencodeAttachChrome` / `ProvidersOpencodeServeChrome` /
/// `ProvidersOpencodeHealthChrome` / `ProvidersOpencodeUsernameChrome`
/// so password chrome stays independently evolvable. OpenCode 2
...
/// password stays data.
pub const ProvidersOpencodePasswordChrome = struct {
    serve_password: []const u8,
    serve_password_description: []const u8,
};
...
/// locale. Same resolve path as ProvidersChrome. Distinct from
/// `ProvidersOpencodeAttachChrome` / `ProvidersOpencodeServeChrome` /
/// `ProvidersOpencodeHealthChrome` / `ProvidersOpencodePasswordChrome`
/// so username chrome stays independently evolvable. OpenCode 2
/// expanded row only (same `show_opencode_attach` gate). For
...
/// username stays data.
pub const ProvidersOpencodeUsernameChrome = struct {
    serve_username: []const u8,
    serve_username_description: []const u8,
};
...
/// Settings → Appearance code font size chrome for the resolved locale.
/// Callers pass Model `language_preference` + `system_locale_id`; this
/// file does not read process env. Distinct from UI font so this pack
/// stays independently evolvable. Chip labels stay Latin.
pub fn appearanceCodeFontChromeFor(preference: LanguagePreference, system_locale_id: []const u8) AppearanceCodeFontChrome {
    return switch (resolve(preference, system_locale_id)) {
        .simplified_chinese => appearance_code_font_chrome_zh_cn,
        .japanese => appearance_code_font_chrome_ja,
        .system, .english => appearance_code_font_chrome_en,
    };
}
...
pub fn reviewDiffGapLabelChromeFor(preference: LanguagePreference, system_locale_id: []const u8) ReviewDiffGapLabelChrome {
    return switch (resolve(preference, system_locale_id)) {
        ...
        .japanese => review_diff_gap_label_chrome_ja,
        .system, .english => review_diff_gap_label_chrome_en,
    };
}

/// Format Review Diff unmodified-line gap label from the pack's
/// singular / plural templates. `count == 1` uses
/// `unmodified_line`; otherwise `unmodified_lines`. Templates
/// start with Latin `{d}`; the rest is locale copy.
pub fn formatReviewDiffGapLabel(chrome: ReviewDiffGapLabelChrome, arena: std.mem.Allocator, count: u32) []const u8 {
...
/// `label=` as the accessible name. Transcript / Conversation
/// history / Review files / Background work / Background output /
/// Browser region a11y live here too (distinct from RightPanelTabs
/// tab chips and ReviewHunkA11yChrome Review hunks).
...
/// Browser toolbar Back / Forward / Reload / Hard Reload / Stop loading /
/// Navigate and Secure / Not secure a11y for the resolved locale. Callers pass
/// Model `language_preference` + `system_locale_id`; this file does
...
/// locale. Callers pass Model `language_preference` +
/// `system_locale_id`; this file does not read process env. Distinct
/// from ProvidersOpencodeAttachChrome / ProvidersOpencodeServeChrome /
/// ProvidersOpencodeHealthChrome / ProvidersOpencodeUsernameChrome so
/// password chrome stays independently evolvable. Wire ids / on-press
/// stay English (`apply_opencode2_password` / `clear_opencode2_password`).
/// Typed password stays data.
pub fn providersOpencodePasswordChromeFor(preference: LanguagePreference, system_locale_id: []const u8) ProvidersOpencodePasswordChrome {
    return switch (resolve(preference, system_locale_id)) {
        .simplified_chinese => providers_opencode_password_chrome_zh_cn,
        .japanese => providers_opencode_password_chrome_ja,
        .system, .english => providers_opencode_password_chrome_en,
    };
}
...
/// `system_locale_id`; this file does not read process env. Distinct
/// from ProvidersOpencodeAttachChrome / ProvidersOpencodeServeChrome /
/// ProvidersOpencodeHealthChrome / ProvidersOpencodePasswordChrome so
/// username chrome stays independently evolvable. Wire ids / on-press
/// stay English (`apply_opencode2_username` / `clear_opencode2_username`).
/// Typed username stays data.
pub fn providersOpencodeUsernameChromeFor(preference: LanguagePreference, system_locale_id: []const u8) ProvidersOpencodeUsernameChrome {
    return switch (resolve(preference, system_locale_id)) {
        .simplified_chinese => providers_opencode_username_chrome_zh_cn,
        .japanese => providers_opencode_username_chrome_ja,
        .system, .english => providers_opencode_username_chrome_en,
    };
}
...
test "appearanceFontChromeFor english default; zh and ja chrome; english ignores ja LANG" {
    ...
    try testing.expectEqualStrings("Text size across chrome and controls", appearanceFontChromeFor(.english, "").description);
    ...
    try testing.expectEqualStrings("界面与控件的文字大小", appearanceFontChromeFor(.simplified_chinese, "").description);
    try testing.expectEqualStrings("UI のフォントサイズ", appearanceFontChromeFor(.japanese, "").title);
    try testing.expectEqualStrings("インターフェースとコントロールの文字サイズ", appearanceFontChromeFor(.japanese, "").description);
...
test "appearanceCodeFontChromeFor english default; zh and ja chrome; english ignores ja LANG" {
    const testing = std.testing;
    try testing.expectEqualStrings("Code font size", appearanceCodeFontChromeFor(.english, "ja").title);
    try testing.expectEqualStrings("Text size for messages, code, diffs, and terminal", appearanceCodeFontChromeFor(.english, "").description);
    try testing.expectEqualStrings("Code font size", appearanceCodeFontChromeFor(.system, "").title);
    try testing.expectEqualStrings("代码字号", appearanceCodeFontChromeFor(.simplified_chinese, "").title);
    try testing.expectEqualStrings("消息、代码、差异和终端的文字大小", appearanceCodeFontChromeFor(.simplified_chinese, "").description);
    try testing.expectEqualStrings("コードのフォントサイズ", appearanceCodeFontChromeFor(.japanese, "").title);
    try testing.expectEqualStrings("メッセージ、コード、差分、ターミナルの文字サイズ", appearanceCodeFontChromeFor(.japanese, "").description);
    try testing.expectEqualStrings("代码字号", appearanceCodeFontChromeFor(.system, "zh_CN.UTF-8").title);
    try testing.expectEqualStrings("コードのフォントサイズ", appearanceCodeFontChromeFor(.system, "ja_JP.UTF-8").title);
    try testing.expectEqualStrings("Code font size", appearanceCodeFontChromeFor(.english, "ja_JP.UTF-8").title);
    try testing.expectEqualStrings("Code font size", appearanceCodeFontChromeFor(.english, "zh_CN.UTF-8").title);
}
...
test "reviewDiffGapLabelChromeFor english default; zh and ja chrome; english ignores ja LANG" {
    const testing = std.testing;
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();

    try testing.expectEqualStrings("{d} unmodified line", reviewDiffGapLabelChromeFor(.english, "ja").unmodified_line);
    try testing.expectEqualStrings("{d} unmodified lines", reviewDiffGapLabelChromeFor(.english, "").unmodified_lines);
    try testing.expectEqualStrings("{d} unmodified line", reviewDiffGapLabelChromeFor(.system, "").unmodified_line);
    try testing.expectEqualStrings("{d} unmodified lines", reviewDiffGapLabelChromeFor(.system, "").unmodified_lines);

    try testing.expectEqualStrings("{d} 行未修改", reviewDiffGapLabelChromeFor(.simplified_chinese, "").unmodified_line);
    try testing.expectEqualStrings("{d} 行未修改", reviewDiffGapLabelChromeFor(.simplified_chinese, "").unmodified_lines);
    try testing.expectEqualStrings("{d} 行未変更", reviewDiffGapLabelChromeFor(.japanese, "").unmodified_line);
    try testing.expectEqualStrings("{d} 行未変更", reviewDiffGapLabelChromeFor(.japanese, "").unmodified_lines);

    try testing.expectEqualStrings("{d} 行未修改", reviewDiffGapLabelChromeFor(.system, "zh_CN.UTF-8").unmodified_line);
    try testing.expectEqualStrings("{d} 行未修改", reviewDiffGapLabelChromeFor(.system, "zh_CN.UTF-8").unmodified_lines);
    try testing.expectEqualStrings("{d} 行未変更", reviewDiffGapLabelChromeFor(.system, "ja_JP.UTF-8").unmodified_line);
    try testing.expectEqualStrings("{d} 行未変更", reviewDiffGapLabelChromeFor(.system, "ja_JP.UTF-8").unmodified_lines);
    try testing.expectEqualStrings("{d} unmodified line", reviewDiffGapLabelChromeFor(.english, "ja_JP.UTF-8").unmodified_line);
    try testing.expectEqualStrings("{d} unmodified lines", reviewDiffGapLabelChromeFor(.english, "zh_CN.UTF-8").unmodified_lines);

    try testing.expectEqualStrings(
        "1 unmodified line",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.english, ""), arena, 1),
    );
    try testing.expectEqualStrings(
        "20 unmodified lines",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.english, ""), arena, 20),
    );
    try testing.expectEqualStrings(
        "1 行未修改",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.simplified_chinese, ""), arena, 1),
    );
    try testing.expectEqualStrings(
        "20 行未修改",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.simplified_chinese, ""), arena, 20),
    );
    try testing.expectEqualStrings(
        "1 行未変更",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.japanese, ""), arena, 1),
    );
    try testing.expectEqualStrings(
        "20 行未変更",
        formatReviewDiffGapLabel(reviewDiffGapLabelChromeFor(.japanese, ""), arena, 20),
    );
    try testing.expectEqualStrings(
        "1 unmodified line",
...
test "structuralRegionChromeFor english default; zh and ja chrome; english ignores ja LANG" {
    ...
    try testing.expectEqualStrings("Transcript", structuralRegionChromeFor(.english, "").transcript);
    try testing.expectEqualStrings("Conversation history", structuralRegionChromeFor(.english, "").conversation_history);
    try testing.expectEqualStrings("Review files", structuralRegionChromeFor(.english, "").review_files);
    try testing.expectEqualStrings("Background work", structuralRegionChromeFor(.english, "").background_work);
    try testing.expectEqualStrings("Background output", structuralRegionChromeFor(.english, "").background_output);
    try testing.expectEqualStrings("Browser", structuralRegionChromeFor(.english, "").browser);
    ...
    try testing.expectEqualStrings("Transcript", structuralRegionChromeFor(.system, "").transcript);
    try testing.expectEqualStrings("Browser", structuralRegionChromeFor(.system, "").browser);
    ...
    try testing.expectEqualStrings("对话记录", structuralRegionChromeFor(.simplified_chinese, "").transcript);
    try testing.expectEqualStrings("对话历史", structuralRegionChromeFor(.simplified_chinese, "").conversation_history);
    try testing.expectEqualStrings("审阅文件", structuralRegionChromeFor(.simplified_chinese, "").review_files);
    try testing.expectEqualStrings("后台工作", structuralRegionChromeFor(.simplified_chinese, "").background_work);
    try testing.expectEqualStrings("后台输出", structuralRegionChromeFor(.simplified_chinese, "").background_output);
    try testing.expectEqualStrings("浏览器", structuralRegionChromeFor(.simplified_chinese, "").browser);
    ...
    try testing.expectEqualStrings("トランスクリプト", structuralRegionChromeFor(.japanese, "").transcript);
    try testing.expectEqualStrings("会話履歴", structuralRegionChromeFor(.japanese, "").conversation_history);
    try testing.expectEqualStrings("レビューファイル", structuralRegionChromeFor(.japanese, "").review_files);
    try testing.expectEqualStrings("バックグラウンド作業", structuralRegionChromeFor(.japanese, "").background_work);
    try testing.expectEqualStrings("バックグラウンド出力", structuralRegionChromeFor(.japanese, "").background_output);
    try testing.expectEqualStrings("ブラウザ", structuralRegionChromeFor(.japanese, "").browser);
    ...
    try testing.expectEqualStrings("对话记录", structuralRegionChromeFor(.system, "zh_CN.UTF-8").transcript);
    try testing.expectEqualStrings("浏览器", structuralRegionChromeFor(.system, "zh_CN.UTF-8").browser);
    ...
    try testing.expectEqualStrings("トランスクリプト", structuralRegionChromeFor(.system, "ja_JP.UTF-8").transcript);
    try testing.expectEqualStrings("ブラウザ", structuralRegionChromeFor(.system, "ja_JP.UTF-8").browser);
    ...
    try testing.expectEqualStrings("Transcript", structuralRegionChromeFor(.english, "zh_CN.UTF-8").transcript);
    try testing.expectEqualStrings("Browser", structuralRegionChromeFor(.english, "ja_JP.UTF-8").browser);
    ...
    try testing.expect(!std.mem.eql(u8, structuralRegionChromeFor(.english, "").review_files, reviewHunkA11yChromeFor(.english, "").review_hunks));
    try testing.expect(!std.mem.eql(u8, structuralRegionChromeFor(.english, "").background_work, rightPanelTabsFor(.english, "").background));
    try testing.expect(!std.mem.eql(u8, structuralRegionChromeFor(.english, "").transcript, structuralRegionChromeFor(.english, "").conversation_history));
    ...
    try testing.expect(!std.mem.eql(u8, structuralRegionChromeFor(.simplified_chinese, "").transcript, structuralRegionChromeFor(.english, "").transcript));
    try testing.expect(!std.mem.eql(u8, structuralRegionChromeFor(.japanese, "").browser, structuralRegionChromeFor(.english, "").browser));
}
...
test "browserToolbarChromeFor english default; zh and ja chrome; english ignores ja LANG" {
    ...
    try testing.expectEqualStrings("Stop loading", browserToolbarChromeFor(.english, "").stop_loading);
    ...
    try testing.expectEqualStrings("停止加载", browserToolbarChromeFor(.simplified_chinese, "").stop_loading);
    ...
    try testing.expectEqualStrings("読み込みを停止", browserToolbarChromeFor(.japanese, "").stop_loading);
    ...
    try testing.expectEqualStrings("Stop loading", browserToolbarChromeFor(.english, "zh_CN.UTF-8").stop_loading);
    ...
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.english, "").stop_loading, composerSendStopChromeFor(.english, "").stop));
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.simplified_chinese, "").stop_loading, composerSendStopChromeFor(.simplified_chinese, "").stop));
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.japanese, "").stop_loading, composerSendStopChromeFor(.japanese, "").stop));
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.english, "").stop_loading, backgroundChromeFor(.english, "").daemon_stop));
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.simplified_chinese, "").stop_loading, backgroundChromeFor(.simplified_chinese, "").daemon_stop));
    try testing.expect(!std.mem.eql(u8, browserToolbarChromeFor(.japanese, "").stop_loading, backgroundChromeFor(.japanese, "").daemon_stop));
}
...
test "providersOpencodePasswordChromeFor english default; zh and ja chrome; Faku not Waku; english ignores ja LANG" {
    const testing = std.testing;
    ...
    try testing.expectEqualStrings("Serve パスワード", providersOpencodePasswordChromeFor(.system, "ja_JP.UTF-8").serve_password);
    try testing.expectEqualStrings("Serve password", providersOpencodePasswordChromeFor(.english, "zh_CN.UTF-8").serve_password);
    try testing.expect(!std.mem.eql(u8, providersOpencodePasswordChromeFor(.english, "").serve_password, providersOpencodeAttachChromeFor(.english, "").attach_url));
    try testing.expect(!std.mem.eql(u8, providersOpencodePasswordChromeFor(.english, "").serve_password, providersOpencodeHealthChromeFor(.english, "").check_serve));
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.english, "").serve_password_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.english, "").serve_password_description, "Waku") == null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.simplified_chinese, "").serve_password_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.simplified_chinese, "").serve_password_description, "Waku") == null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.japanese, "").serve_password_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodePasswordChromeFor(.japanese, "").serve_password_description, "Waku") == null);
}

test "providersOpencodeUsernameChromeFor english default; zh and ja chrome; Faku not Waku; english ignores ja LANG" {
    const testing = std.testing;
    ...
    try testing.expectEqualStrings("Serve ユーザー名", providersOpencodeUsernameChromeFor(.system, "ja_JP.UTF-8").serve_username);
    try testing.expectEqualStrings("Serve username", providersOpencodeUsernameChromeFor(.english, "zh_CN.UTF-8").serve_username);
    try testing.expect(!std.mem.eql(u8, providersOpencodeUsernameChromeFor(.english, "").serve_username, providersOpencodePasswordChromeFor(.english, "").serve_password));
    try testing.expect(!std.mem.eql(u8, providersOpencodeUsernameChromeFor(.english, "").serve_username, providersOpencodeAttachChromeFor(.english, "").attach_url));
    try testing.expect(!std.mem.eql(u8, providersOpencodeUsernameChromeFor(.english, "").serve_username, providersOpencodeHealthChromeFor(.english, "").check_serve));
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.english, "").serve_username_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.english, "").serve_username_description, "Waku") == null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.simplified_chinese, "").serve_username_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.simplified_chinese, "").serve_username_description, "Waku") == null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.japanese, "").serve_username_description, "Faku") != null);
    try testing.expect(std.mem.indexOf(u8, providersOpencodeUsernameChromeFor(.japanese, "").serve_username_description, "Waku") == null);
}
