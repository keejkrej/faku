...
fn expectSessionContextMenu(tree: AppUi.Tree, row: canvas.Widget, session_id: u32) !void {
    ...
    if (@hasDecl(@TypeOf(tree), "msgForContextMenu")) {
        ...
        try testing.expectEqual(Msg{ .remove_session = session_id }, tree.msgForContextMenu(row.id, 2).?);
    }
}
...
test "fork copies turns and project_path; new id; empty fx_session_id; source unchanged; empty is a no-op" {
    ...
    if (model.sessionById(id)) |session| {
        ...
        session.setWorktreeTurnDiffSha("dddddddddddddddddddddddddddddddddddddddd");
    }
    model.selected = id;
    ...
    try testing.expectEqualStrings("dddddddddddddddddddddddddddddddddddddddd", forked.worktreeTurnDiffSha());
    ...
    try testing.expectEqualStrings("dddddddddddddddddddddddddddddddddddddddd", source.worktreeTurnDiffSha());
    ...
    try testing.expectEqualStrings("dddddddddddddddddddddddddddddddddddddddd", loaded_fork.worktreeTurnDiffSha());
...
test "pick_folder stdout directory sets project_path the same way typing does" {
    ...
    try testing.expectEqual(@as(usize, 0), loaded.review_diff_hunk_len);
    try testing.expectEqual(@as(u64, 0), loaded.next_review_diff_hunk_key - review_diff.review_diff_hunk_key_first);
}
...
fn findOpenTerminalSpawn(fx: *Effects) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (open_terminal.isTerminalArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn findOpenTerminalSpawnNamed(fx: *Effects, bin: []const u8) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.argv.len > 0 and std.mem.eql(u8, spawn.argv[0], bin) and open_terminal.isTerminalArgv(spawn.argv)) {
            return spawn;
        }
    }
    return null;
...
fn absOpenTerminalDir(tmp: testing.TmpDir, name: []const u8, buf: []u8) ![]const u8 {
    const path = try std.fmt.bufPrint(buf, "/tmp/faku-term-{s}-{s}", .{ tmp.sub_path, name });
    try std.Io.Dir.cwd().createDirPath(testing.io, path);
    return path;
}

test "open_terminal existing directory captures host terminal argv and leaves project_path" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenTerminalDir(tmp, "ok", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("open terminal session", .fx);
    model.selected = id;
    model.setSelectedProjectPath(project);
    ...
    const before = model.selectedProjectPath();
    main.update(&model, tree.msgForPointer(terminal.id, .up).?, &fx);
    if (open_terminal.hostBin(.first) == null) {
        try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
        try testing.expectEqualStrings(open_terminal.hostMissingStatus(), model.window_status());
        try testing.expectEqualStrings(before, model.selectedProjectPath());
        return;
    }
    ...
    try testing.expect(!reveal_folder.isRevealArgv(spawn.argv));
    try testing.expect(!pick_folder.isPickerArgv(spawn.argv));
    try testing.expectEqualStrings(open_terminal.hostBin(.first).?, spawn.argv[0]);
    if (spawn.argv.len == 4) {
        ...
        try testing.expect(std.mem.endsWith(u8, spawn.argv[1], project));
    }
    try testing.expectEqualStrings("", spawn.stdin);
    try testing.expectEqualStrings(project, model.selectedProjectPath());
    try testing.expectEqualStrings(project, model.lastProjectPath());
...
test "open_terminal empty missing file Local and relative are no-ops" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var file_buf: [256]u8 = undefined;
    const file_path = try std.fmt.bufPrint(&file_buf, "/tmp/faku-term-file-{s}.txt", .{tmp.sub_path});
    try std.Io.Dir.cwd().writeFile(testing.io, .{ .sub_path = file_path, .data = "nope" });
    var rel_buf: [256]u8 = undefined;
    const rel = try std.fmt.bufPrint(&rel_buf, ".zig-cache/tmp/{s}/rel-term", .{tmp.sub_path});
    try std.Io.Dir.cwd().createDirPath(testing.io, rel);
    var missing_buf: [256]u8 = undefined;
    const missing = try std.fmt.bufPrint(&missing_buf, "/tmp/faku-term-missing-{s}", .{tmp.sub_path});
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("no terminal", .fx);
    model.selected = id;
    ...
    try testing.expect(model.project_is_local());
    try testing.expect(!model.can_open_terminal());
    main.update(&model, .open_terminal, &fx);
    try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
    try testing.expectEqualStrings(open_terminal.no_project_status, model.window_status());
    try testing.expectEqual(@as(usize, 0), model.selectedProjectPath().len);
...
test "open_terminal sits on idle and project-edit rows only when the path is openable" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenTerminalDir(tmp, "row", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("terminal row", .fx);
    model.selected = id;
...
test "palette Open project in Terminal runs the same handler" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenTerminalDir(tmp, "palette", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("terminal palette", .fx);
    model.selected = id;
...
test "in-flight second Open in Terminal is a no-op; missing tool surfaces status" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenTerminalDir(tmp, "live", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("terminal live", .fx);
    model.selected = id;
    model.setSelectedProjectPath(project);
    ...
    const first = findOpenTerminalSpawn(&fx) orelse return error.MissingOpenTerminalSpawn;
    try fx.feedExit(first.key, 127);
    drainEffects(&model, &fx);

    if (open_terminal.hostBin(.fallback)) |fallback_bin| {
        try testing.expect(model.open_terminal_live);
        const second = findOpenTerminalSpawnNamed(&fx, fallback_bin) orelse return error.MissingFallbackSpawn;
        try testing.expect(open_terminal.isTerminalArgv(second.argv));
        try testing.expect(!reveal_folder.isRevealArgv(second.argv));
        try fx.feedExit(second.key, 127);
        drainEffects(&model, &fx);
    }
...
fn findOpenEditorSpawn(fx: *Effects) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (open_editor.isEditorArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn findOpenEditorSpawnNamed(fx: *Effects, bin: []const u8) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.argv.len > 0 and std.mem.eql(u8, spawn.argv[0], bin) and open_editor.isEditorArgv(spawn.argv)) {
            return spawn;
        }
    }
    return null;
...
fn absOpenEditorDir(tmp: testing.TmpDir, name: []const u8, buf: []u8) ![]const u8 {
    const path = try std.fmt.bufPrint(buf, "/tmp/faku-editor-{s}-{s}", .{ tmp.sub_path, name });
    try std.Io.Dir.cwd().createDirPath(testing.io, path);
    return path;
}

test "open_editor existing directory captures host editor argv and leaves project_path" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenEditorDir(tmp, "ok", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("open editor session", .fx);
    model.selected = id;
    model.setSelectedProjectPath(project);
    ...
    const before = model.selectedProjectPath();
    main.update(&model, tree.msgForPointer(editor.id, .up).?, &fx);
    if (open_editor.hostBin(.first) == null) {
        try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
        try testing.expectEqualStrings(open_editor.hostMissingStatus(), model.window_status());
        try testing.expectEqualStrings(before, model.selectedProjectPath());
        return;
    }
    ...
    try testing.expect(!reveal_folder.isRevealArgv(spawn.argv));
    try testing.expect(!pick_folder.isPickerArgv(spawn.argv));
    try testing.expectEqualStrings(open_editor.hostBin(.first).?, spawn.argv[0]);
    try testing.expectEqual(@as(usize, 2), spawn.argv.len);
    try testing.expectEqualStrings(project, spawn.argv[1]);
    try testing.expectEqualStrings("", spawn.stdin);
    try testing.expectEqualStrings(project, model.selectedProjectPath());
...
test "open_editor empty missing file Local and relative are no-ops" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var file_buf: [256]u8 = undefined;
    const file_path = try std.fmt.bufPrint(&file_buf, "/tmp/faku-editor-file-{s}.txt", .{tmp.sub_path});
    try std.Io.Dir.cwd().writeFile(testing.io, .{ .sub_path = file_path, .data = "nope" });
    var rel_buf: [256]u8 = undefined;
    const rel = try std.fmt.bufPrint(&rel_buf, ".zig-cache/tmp/{s}/rel-editor", .{tmp.sub_path});
    try std.Io.Dir.cwd().createDirPath(testing.io, rel);
    var missing_buf: [256]u8 = undefined;
    const missing = try std.fmt.bufPrint(&missing_buf, "/tmp/faku-editor-missing-{s}", .{tmp.sub_path});
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("no editor", .fx);
    model.selected = id;
    ...
    try testing.expect(model.project_is_local());
    try testing.expect(!model.can_open_editor());
    main.update(&model, .open_editor, &fx);
    try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
    try testing.expectEqualStrings(open_editor.no_project_status, model.window_status());
    try testing.expectEqual(@as(usize, 0), model.selectedProjectPath().len);
...
test "open_editor sits on idle and project-edit rows only when the path is openable" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenEditorDir(tmp, "row", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("editor row", .fx);
    model.selected = id;
...
test "palette Open project in Editor runs the same handler" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenEditorDir(tmp, "palette", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("editor palette", .fx);
    model.selected = id;
...
test "in-flight second Open in Editor is a no-op; missing tool surfaces status" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absOpenEditorDir(tmp, "live", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("editor live", .fx);
    model.selected = id;
    model.setSelectedProjectPath(project);
    ...
    const first = findOpenEditorSpawn(&fx) orelse return error.MissingOpenEditorSpawn;
    try fx.feedExit(first.key, 127);
    drainEffects(&model, &fx);

    if (open_editor.hostBin(.fallback)) |fallback_bin| {
        try testing.expect(model.open_editor_live);
        const second = findOpenEditorSpawnNamed(&fx, fallback_bin) orelse return error.MissingFallbackSpawn;
        try testing.expect(open_editor.isEditorArgv(second.argv));
        try testing.expect(!open_terminal.isTerminalArgv(second.argv));
        try testing.expect(!reveal_folder.isRevealArgv(second.argv));
        try fx.feedExit(second.key, 127);
        drainEffects(&model, &fx);
    ...
    if (open_editor.hostBin(.macos_cursor_app)) |macos_bin| {
        try testing.expect(model.open_editor_live);
        const third = findOpenEditorSpawnNamed(&fx, macos_bin) orelse return error.MissingMacosCursorSpawn;
        try testing.expect(open_editor.isEditorArgv(third.argv));
        try testing.expectEqualStrings(open_editor.macos_cursor_app, third.argv[2]);
        try fx.feedExit(third.key, 127);
        drainEffects(&model, &fx);
    }

    if (open_editor.hostBin(.macos_vscode_app)) |macos_bin| {
        try testing.expect(model.open_editor_live);
        const fourth = findOpenEditorSpawnNamed(&fx, macos_bin) orelse return error.MissingMacosVscodeSpawn;
        try testing.expect(open_editor.isEditorArgv(fourth.argv));
        try testing.expectEqualStrings(open_editor.macos_vscode_app, fourth.argv[2]);
        try fx.feedExit(fourth.key, 127);
        drainEffects(&model, &fx);
    }
...
fn absCopyProjectDir(tmp: testing.TmpDir, name: []const u8, buf: []u8) ![]const u8 {
    const path = try std.fmt.bufPrint(buf, "/tmp/faku-copy-path-{s}-{s}", .{ tmp.sub_path, name });
    try std.Io.Dir.cwd().createDirPath(testing.io, path);
    return path;
}

test "copy_project_path existing directory writes fx.writeClipboard and leaves project_path" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absCopyProjectDir(tmp, "ok", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("copy project path session", .fx);
    model.selected = id;
    model.setSelectedProjectPath(project);
    ...
    const before = model.selectedProjectPath();
    try testing.expectEqual(@as(usize, 0), fx.pendingClipboardCount());
    main.update(&model, tree.msgForPointer(copy_path.id, .up).?, &fx);
    try testing.expectEqual(@as(usize, 1), fx.pendingClipboardCount());
    try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
    const first = fx.pendingClipboardAt(0).?;
    try testing.expectEqual(main.copy_turn_key, first.key);
...
test "copy_project_path empty missing file Local and relative are no-ops" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var file_buf: [256]u8 = undefined;
    const file_path = try std.fmt.bufPrint(&file_buf, "/tmp/faku-copy-path-file-{s}.txt", .{tmp.sub_path});
    try std.Io.Dir.cwd().writeFile(testing.io, .{ .sub_path = file_path, .data = "nope" });
    var rel_buf: [256]u8 = undefined;
    const rel = try std.fmt.bufPrint(&rel_buf, ".zig-cache/tmp/{s}/rel-copy-path", .{tmp.sub_path});
    try std.Io.Dir.cwd().createDirPath(testing.io, rel);
    var missing_buf: [256]u8 = undefined;
    const missing = try std.fmt.bufPrint(&missing_buf, "/tmp/faku-copy-path-missing-{s}", .{tmp.sub_path});
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("no copy path", .fx);
    model.selected = id;
    ...
    try testing.expect(model.project_is_local());
    try testing.expect(!model.can_copy_project_path());
    main.update(&model, .copy_project_path, &fx);
    try testing.expectEqual(@as(usize, 0), fx.pendingClipboardCount());
    try testing.expectEqual(@as(usize, 0), fx.pendingSpawnCount());
    try testing.expectEqual(@as(usize, 0), model.selectedProjectPath().len);
...
test "copy_project_path sits on idle and project-edit rows only when the path is openable" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absCopyProjectDir(tmp, "row", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("copy path row", .fx);
    model.selected = id;
...
test "palette Copy project path runs the same handler" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try absCopyProjectDir(tmp, "palette", &project_buf);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("copy path palette", .fx);
    model.selected = id;
...
test "Send records the pre-commit HEAD; a later commit stays off the rewind target" {
    ...
    try testing.expectEqual(@as(usize, 0), at_send.worktreeTurnDiffSha().len);
    ...
    try testing.expect(rewind.isStoredSha(live.worktreeTurnDiffSha()));
    try testing.expectEqualStrings(live.worktreeSnapshotSha(), live.worktreeTurnDiffSha());
    ...
    try testing.expectEqualStrings(live.worktreeTurnDiffSha(), loaded.session_store[0].worktreeTurnDiffSha());
...
test "Send worktree snapshot includes dirty and untracked; overwrite keeps latest" {
    ...
    try testing.expectEqual(@as(usize, 0), model.sessionById(id).?.worktreeTurnDiffSha().len);
    ...
    try testing.expectEqualStrings(first_owned, model.sessionById(id).?.worktreeTurnDiffSha());
...
test "Send names the worktree snapshot under refs/faku" {
    ...
    var first_diff_buf: [checkpoint.max_faku_ref_name]u8 = undefined;
    const first_diff = checkpoint.formatFakuSessionTurnDiffRef(&first_diff_buf, id, 1) orelse return error.MissingFakuDiffRef;
    try testing.expectEqualStrings("refs/faku/session-1-turn-diff-1", first_diff);
    try testing.expect(checkpoint.hasFakuRef(allocator, testing.io, project, first_diff));
    const first_diff_parsed = try runGitCapture(allocator, testing.io, &.{ "git", "-C", project, "rev-parse", first_diff });
    defer allocator.free(first_diff_parsed);
    try testing.expectEqualStrings(first_owned, std.mem.trim(u8, first_diff_parsed, " \r\n\t"));
    ...
    var second_diff_buf: [checkpoint.max_faku_ref_name]u8 = undefined;
    const second_diff = checkpoint.formatFakuSessionTurnDiffRef(&second_diff_buf, id, 2) orelse return error.MissingFakuDiffRef;
    try testing.expect(checkpoint.hasFakuRef(allocator, testing.io, project, second_diff));
    try testing.expect(model.is_streaming());
    ...
    var third_diff_buf: [checkpoint.max_faku_ref_name]u8 = undefined;
    const third_diff = checkpoint.formatFakuSessionTurnDiffRef(&third_diff_buf, id, 3) orelse return error.MissingFakuDiffRef;
    try testing.expect(!checkpoint.hasFakuRef(allocator, testing.io, project, third_diff));
    main.update(&model, .stop, &fx);
    try testing.expect(!model.is_streaming());
    try testing.expect(!checkpoint.hasFakuRef(allocator, testing.io, project, third_end));
    try testing.expect(!checkpoint.hasFakuRef(allocator, testing.io, project, third_diff));
...
test "Send turn-start snapshot has parents and metadata; finish end snapshot does not" {
    ...
    try testing.expectEqualStrings(start, model.sessionById(id).?.worktreeTurnDiffSha());
...
test "Rewind restores Send-time files, pops that ref, and truncates the last prompt" {
    ...
    try testing.expectEqual(@as(usize, 0), after_one.worktreeTurnDiffSha().len);
    try testing.expectEqual(@as(u32, 2), model.turnCount(id));
    try testing.expectEqual(@as(usize, 1), countRole(&model, .user));
...
test "right panel Files and Diff tabs switch surfaces and Diff opens the pane" {
    ...
    try testing.expect(model.right_panel_tab_files());
    try testing.expectEqual(@as(u32, 360), model.rightPanelWidthPixels());
    try testing.expect(model.review_diff_active);
    tree = try buildTree(arena, &model);
    try testing.expect(findByText(tree.root, .text, "Review") == null);
    _ = try expectByText(tree.root, .text, "No project open");
...
test "palette copies local session id and fx session id; empty fx id skips clipboard" {
    ...
    try testing.expect(paletteRowIsAction(empty, "Copy session id"));
    try testing.expect(paletteRowIsAction(empty, "Copy provider session id"));
    ...
    try testing.expectEqualStrings("fx-sess-palette", model.sessionById(selected).?.fxSessionId());
    ...
    var local_fx = Effects.init(testing.allocator);
    defer local_fx.deinit();
    local_fx.executor = .fake;
    main.update(&model, .start_search, &local_fx);
    tree = try buildTree(arena, &model);
    const pick_local = try expectButton(
        findByKind(tree.root, .dialog) orelse return error.WidgetNotFound,
        "Copy session id",
    );
    main.update(&model, tree.msgForPointer(pick_local.id, .up).?, &local_fx);
    try testing.expect(!model.palette_open);
...
test "sidebar session rows declare a Rename/Remove context menu" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var dir_buf: [256]u8 = undefined;
    const dir = try std.fmt.bufPrint(&dir_buf, ".zig-cache/tmp/{s}/faku-session-context-menu", .{tmp.sub_path[0..]});
    ...
    var tree = try buildTree(arena, &model);
    const today_row = try expectByText(tree.root, .list_item, "port waku to zig");
    try testing.expectEqual(Msg{ .select = port_id }, tree.msgForPointer(today_row.id, .up).?);
    try expectSessionContextMenu(tree, today_row, port_id);
    const other_today = try expectByText(tree.root, .list_item, "fix auth listener");
    try testing.expectEqual(Msg{ .select = auth_id }, tree.msgForPointer(other_today.id, .up).?);
    ...
    try testing.expectEqual(auth_id, model.editing_session_id);
    try testing.expectEqual(port_id, model.selected);
    try testing.expectEqualStrings("fix auth listener", model.session_title_draft());
    try testing.expect(!model.composer_active);

    tree = try buildTree(arena, &model);
    try testing.expect(findByPlaceholder(tree.root, .text_field, "untitled") != null);
    const editing_row = try expectByText(tree.root, .list_item, "fix auth listener");
    try expectSessionContextMenu(tree, editing_row, auth_id);

    main.update(&model, .{ .session_title_edit = .clear }, &fx);
    main.update(&model, .{ .session_title_edit = .{ .insert_text = "auth rename" } }, &fx);
    try testing.expectEqualStrings("auth rename", model.session_store[1].title());
    try testing.expectEqual(auth_id, model.editing_session_id);
    try testing.expectEqual(port_id, model.selected);
    ...
    loaded.store_io = testing.io;
    try testing.expectEqual(store.LoadKind.loaded, store.loadCatalog(&loaded, testing.allocator, testing.io));
    try testing.expectEqualStrings("auth rename", loaded.session_store[1].title());
...
test "composer project row shows one-shot git branch --show-current" {
    ...
    const tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .button, project);
    _ = try expectByText(tree.root, .select, "feat/composer-row");
    try testing.expect(findByText(tree.root, .button, "Local") == null);
}
...
fn findGitBranchListSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_checkout.isGitBranchListArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn findGitCheckoutSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_checkout.isGitCheckoutArgv(spawn.argv)) return spawn;
    }
    return null;
}
...
test "composer project row lists local heads and checks out another branch" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-co", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("checkout row", .fx);
    model.selected = id;
    ...
    try testing.expect(list.key != branch.key);
    ...
    try testing.expectEqual(Msg.toggle_git_branch_picker, tree.msgForPointer(chip.id, .up).?);
    ...
    main.update(&model, .toggle_git_branch_picker, &fx);
    main.update(&model, .{ .pick_git_branch = "feat/a" }, &fx);
    try testing.expect(!model.git_branch_picker_open);
    const checkout = findGitCheckoutSpawnKey(&fx, model.git_checkout_key) orelse return error.MissingGitCheckoutSpawn;
    try expectGitCheckoutArgv(checkout, project, "feat/a");
    try testing.expectEqualStrings("main", model.git_branch_label());

    try fx.feedExit(checkout.key, 0);
    drainEffects(&model, &fx);
...
test "git checkout failure sets composer status and keeps the previous branch" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-co-fail", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("checkout fail", .fx);
    model.selected = id;
    ...
    main.update(&model, .{ .pick_git_branch = "../escape" }, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_checkout_key);
    try testing.expectEqualStrings("main", model.git_branch_label());

    main.update(&model, .{ .pick_git_branch = "feat/blocked" }, &fx);
    const checkout = findGitCheckoutSpawnKey(&fx, model.git_checkout_key) orelse return error.MissingGitCheckoutSpawn;
    try expectGitCheckoutArgv(checkout, project, "feat/blocked");
    try fx.feedExit(checkout.key, 1);
    drainEffects(&model, &fx);
    try testing.expectEqualStrings("main", model.git_branch_label());
    try testing.expectEqualStrings(git_checkout.checkout_failed_status, model.attach_status());
    try testing.expect(model.has_attach_status());
}

test "detached HEAD still opens the local branch picker when heads exist" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-detach", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("detached", .fx);
    model.selected = id;

    main.update(&model, .{ .project_path_edit = .{ .insert_text = project } }, &fx);
    const branch = findGitBranchSpawnKey(&fx, model.git_branch_key) orelse return error.MissingGitBranchSpawn;
    try fx.feedExit(branch.key, 0);
    drainEffects(&model, &fx);
    try testing.expect(!model.has_git_branch());
    try testing.expect(model.git_branch_probe_is_rev_parse);
    const sha = findGitRevParseSpawnKey(&fx, model.git_branch_key) orelse return error.MissingGitRevParseSpawn;
    try testing.expect(git_branch.isGitRevParseArgv(sha.argv));
    ...
    const tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .select, "a1b2c3d");
...
fn findGitDeleteSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_checkout.isGitDeleteArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn expectGitDeleteArgv(spawn: anytype, cwd: []const u8, name: []const u8) !void {
    try testing.expect(git_checkout.isGitDeleteArgv(spawn.argv));
    ...
    try testing.expectEqualStrings(git_checkout.git_delete_d_flag, spawn.argv[7]);
    try testing.expectEqualStrings(name, spawn.argv[8]);
    try testing.expect(std.mem.indexOf(u8, spawn.argv[2], name) == null);
...
test "Delete branch opens delete UI; Esc and cancel close it; current is omitted" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-del-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("delete ui", .fx);
    model.selected = id;
    ...
    tree = try buildTree(arena, &model);
    try testing.expect(findByKind(tree.root, .dropdown_menu) == null);
    const force_off = try expectButtonMsg(tree, "Force", .toggle_git_branch_delete_force);
    try testing.expect(!force_off.state.selected);
    try testing.expect(!model.git_branch_delete_force);
    _ = try expectButtonMsg(tree, "Delete", .confirm_git_branch_delete);
    _ = try expectButtonMsg(tree, "Cancel", .cancel_git_branch_delete);

    main.update(&model, tree.msgForPointer(force_off.id, .up).?, &fx);
    try testing.expect(model.git_branch_delete_force);
    tree = try buildTree(arena, &model);
    const force_on = try expectButtonMsg(tree, "Force", .toggle_git_branch_delete_force);
    try testing.expect(force_on.state.selected);

    main.update(&model, .toggle_git_branch_delete_picker, &fx);
    try testing.expect(model.git_branch_delete_picker_open);
    tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .menu_item, "feat/old");
    try testing.expect(findByText(tree.root, .menu_item, "main") == null);
    ...
    try testing.expect(!model.git_branch_delete_active);
    try testing.expect(!model.git_branch_delete_force);
    try testing.expectEqual(@as(u64, 0), model.git_delete_key);
    tree = try buildTree(arena, &model);
    try testing.expect(findByText(tree.root, .button, "Delete") == null);
    ...
    main.update(&model, .start_git_branch_delete, &fx);
    try testing.expect(!model.git_branch_delete_force);
    main.update(&model, .{ .pick_git_branch_delete = "feat/old" }, &fx);
    try testing.expectEqualStrings("feat/old", model.git_branch_delete());
    main.update(&model, .cancel_git_branch_delete, &fx);
    try testing.expect(!model.git_branch_delete_active);
    try testing.expectEqualStrings("", model.git_branch_delete());
    try testing.expect(!model.git_branch_delete_force);
...
test "confirm delete spawns branch -d; success refreshes; failure sets status" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-del-co", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("delete branch", .fx);
    model.selected = id;
    ...
    main.update(&model, .start_git_branch_delete, &fx);
    try testing.expect(model.git_branch_delete_active);
    main.update(&model, .{ .pick_git_branch_delete = "main" }, &fx);
    try testing.expectEqualStrings("", model.git_branch_delete());
    main.update(&model, .confirm_git_branch_delete, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_delete_key);
    try testing.expect(model.git_branch_delete_active);

    main.update(&model, .{ .pick_git_branch_delete = "../escape" }, &fx);
    main.update(&model, .confirm_git_branch_delete, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_delete_key);
    ...
    model.phase = .idle;
    main.update(&model, .confirm_git_branch_delete, &fx);
    const deleted = findGitDeleteSpawnKey(&fx, model.git_delete_key) orelse return error.MissingGitDeleteSpawn;
    try expectGitDeleteArgv(deleted, project, "feat/old");
    try testing.expect(model.git_branch_delete_active);
    try testing.expectEqualStrings("main", model.git_branch_label());

    const delete_key = model.git_delete_key;
    main.update(&model, .confirm_git_branch_delete, &fx);
    try testing.expectEqual(delete_key, model.git_delete_key);
    main.update(&model, .{ .pick_git_branch = "feat/old" }, &fx);
    try testing.expectEqual(delete_key, model.git_delete_key);
    try testing.expectEqual(@as(u64, 0), model.git_checkout_key);

    try fx.feedExit(deleted.key, 0);
    drainEffects(&model, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_delete_key);
    try testing.expect(!model.git_branch_delete_active);
    try testing.expectEqualStrings("", model.git_branch_delete());
    ...
    main.update(&model, .start_git_branch_delete, &fx);
    main.update(&model, .{ .pick_git_branch_delete = "feat/blocked" }, &fx);
    main.update(&model, .confirm_git_branch_delete, &fx);
    const failed = findGitDeleteSpawnKey(&fx, model.git_delete_key) orelse return error.MissingGitDeleteFailSpawn;
    try expectGitDeleteArgv(failed, project, "feat/blocked");
    try fx.feedExit(failed.key, 1);
    drainEffects(&model, &fx);
    try testing.expectEqualStrings("main", model.git_branch_label());
...
fn findGitDeleteForceSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_checkout.isGitDeleteForceArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn expectGitDeleteForceArgv(spawn: anytype, cwd: []const u8, name: []const u8) !void {
    try testing.expect(git_checkout.isGitDeleteForceArgv(spawn.argv));
    try testing.expect(!git_checkout.isGitDeleteArgv(spawn.argv));
    try testing.expectEqualStrings(git_checkout.sh_bin, spawn.argv[0]);
    try testing.expectEqualStrings(main.fx_ask_chdir_script, spawn.argv[2]);
    ...
    try testing.expectEqualStrings(git_checkout.git_bin, spawn.argv[5]);
    try testing.expectEqualStrings(git_checkout.git_branch_cmd, spawn.argv[6]);
    try testing.expectEqualStrings(git_checkout.git_delete_force_flag, spawn.argv[7]);
    try testing.expectEqualStrings(name, spawn.argv[8]);
    try testing.expect(std.mem.indexOf(u8, spawn.argv[2], name) == null);
...
test "confirm delete Force spawns branch -D; success refreshes; failure keeps card" {
    var tmp = testing.tmpDir(.{});
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-del-force-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("delete force branch", .fx);
    model.selected = id;
    ...
    try testing.expect(!model.git_branch_delete_force);
    main.update(&model, .{ .pick_git_branch_delete = "feat/old" }, &fx);
    main.update(&model, .toggle_git_branch_delete_force, &fx);
    try testing.expect(model.git_branch_delete_force);
    model.phase = .streaming;
    main.update(&model, .confirm_git_branch_delete, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_delete_key);
    model.phase = .idle;
    main.update(&model, .confirm_git_branch_delete, &fx);
    const deleted = findGitDeleteForceSpawnKey(&fx, model.git_delete_key) orelse return error.MissingGitDeleteForceSpawn;
    try expectGitDeleteForceArgv(deleted, project, "feat/old");
    try testing.expect(findGitDeleteSpawnKey(&fx, model.git_delete_key) == null);
    try testing.expect(model.git_branch_delete_active);
    ...
    main.update(&model, .toggle_git_branch_delete_force, &fx);
    main.update(&model, .confirm_git_branch_delete, &fx);
    const failed = findGitDeleteForceSpawnKey(&fx, model.git_delete_key) orelse return error.MissingGitDeleteForceFailSpawn;
    try expectGitDeleteForceArgv(failed, project, "feat/blocked");
    try fx.feedExit(failed.key, 1);
    drainEffects(&model, &fx);
    try testing.expectEqualStrings("main", model.git_branch_label());
    try testing.expectEqualStrings(git_checkout.delete_failed_status, model.attach_status());
...
fn findGitPushSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_checkout.isGitPushArgv(spawn.argv)) return spawn;
    }
    return null;
}
...
test "Push menu item one-shots git push; success refreshes; failure sets status" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-push", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("push remotes", .fx);
    const other = model.addSession("other push", .fx);
    model.selected = id;
    ...
    const push_key = model.git_push_key;
    main.update(&model, .start_git_push, &fx);
    try testing.expectEqual(push_key, model.git_push_key);
    main.update(&model, .start_git_fetch, &fx);
    try testing.expectEqual(push_key, model.git_push_key);
    try testing.expectEqual(@as(u64, 0), model.git_fetch_key);
    main.update(&model, .{ .pick_git_branch = "main" }, &fx);
...
fn findGitWorktreeAddSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
...
fn findGitWorktreeBaseSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
...
fn expectGitWorktreeBaseArgv(spawn: anytype, cwd: []const u8) !void {
    ...
    try testing.expectEqualStrings(git_checkout.git_quiet_flag, spawn.argv[7]);
...
fn finishWorktreeBaseProbe(fx: *Effects, model: *Model, stdout: []const u8, code: u8) !void {
    const probe = findGitWorktreeBaseSpawnKey(fx, model.git_worktree_base_key) orelse return error.MissingGitWorktreeBaseSpawn;
    try expectGitWorktreeBaseArgv(probe, model.selectedProjectPath());
    if (stdout.len > 0) try fx.feedLine(probe.key, stdout);
    drainEffects(model, fx);
    try fx.feedExit(probe.key, code);
    drainEffects(model, fx);
}
...
fn expectGitWorktreeAddArgv(spawn: anytype, cwd: []const u8, home: []const u8, name: []const u8, base: []const u8) !void {
    ...
    try testing.expect(git_checkout.isGitWorktreeAddArgv(spawn.argv));
    if (base.len == 0) {
        try testing.expectEqual(@as(usize, 12), spawn.argv.len);
    } else {
        try testing.expectEqual(@as(usize, 13), spawn.argv.len);
        try testing.expectEqualStrings(base, spawn.argv[12]);
        try testing.expect(std.mem.indexOf(u8, spawn.argv[2], base) == null);
    }
    try testing.expectEqualStrings(git_checkout.sh_bin, spawn.argv[0]);
    try testing.expectEqualStrings(git_checkout.git_worktree_mkdir_chdir_script, spawn.argv[2]);
    try testing.expectEqualStrings(parent, spawn.argv[4]);
    try testing.expectEqualStrings(cwd, spawn.argv[5]);
    try testing.expectEqualStrings(git_checkout.git_bin, spawn.argv[6]);
    try testing.expectEqualStrings(git_checkout.git_worktree_cmd, spawn.argv[7]);
    try testing.expectEqualStrings(git_checkout.git_worktree_add_cmd, spawn.argv[8]);
    try testing.expectEqualStrings(git_checkout.git_create_b_flag, spawn.argv[9]);
    try testing.expectEqualStrings(branch, spawn.argv[10]);
...
test "New worktree opens create UI; Esc and cancel close it" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-wt-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("worktree ui", .fx);
    model.selected = id;
    ...
    try testing.expectEqual(Msg.start_git_worktree_create, tree.msgForPointer(new_item.id, .up).?);
    _ = try expectByText(tree.root, .menu_item, "New branch…");
...
test "Commit card shows Committing and Committing and pushing pending labels" {
    var arena_state = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-commit-pending-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("commit pending ui", .fx);
    model.selected = id;
    if (model.sessionById(id)) |session| session.setProjectPath(project);
    model.git_dirty_count = 1;
    model.git_dirty_key = 0;
    ...
    main.update(&model, .start_git_commit, &fx);
    main.update(&model, .{ .git_commit_edit = .{ .insert_text = "ship the dirty probe" } }, &fx);
    main.update(&model, .confirm_git_commit_and_push, &fx);
    try testing.expect(model.git_commit_then_push);
    try testing.expect(model.has_git_commit_committing_and_pushing());
    try testing.expect(!model.has_git_commit_committing());
    ...
    try testing.expect(model.has_git_commit_committing_and_pushing());
    const commit = findPendingGitCommitCmd(&fx, model.git_commit_key) orelse return error.MissingGitCommitSpawn;
    git_commit.handleCommitExit(&model, &fx, .{ .key = commit.key, .reason = .exited, .code = 0 });
    try testing.expect(model.git_commit_active);
    try testing.expect(model.git_commit_then_push);
    try testing.expect(model.has_git_commit_pushing());
    try testing.expect(model.has_git_commit_committing_and_pushing());
...
test "confirm New worktree one-shots git worktree add -b; success retargets project_path; failure sets status" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/git-wt-add", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    var home_buf: [256]u8 = undefined;
    const home = try std.fmt.bufPrint(&home_buf, "/tmp/faku-wt-home-{s}", .{tmp.sub_path[0..]});
    ...
    main.update(&model, .start_git_worktree_create, &fx);
    try testing.expect(model.git_worktree_create_active);
    try testing.expectEqualStrings("worktree-add", model.git_worktree_create());
    main.update(&model, .{ .git_worktree_create_edit = .{ .insert_text = "not a branch" } }, &fx);
    ...
    main.update(&model, .{ .git_worktree_create_edit = .clear }, &fx);
    main.update(&model, .{ .git_worktree_create_edit = .{ .insert_text = "feat/foo" } }, &fx);
    main.update(&model, .confirm_git_worktree_create, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_worktree_add_key);
    try testing.expectEqual(@as(u64, 0), model.git_worktree_base_key);

    main.update(&model, .{ .git_worktree_create_edit = .clear }, &fx);
    main.update(&model, .{ .git_worktree_create_edit = .{ .insert_text = "feat-new" } }, &fx);
    model.phase = .streaming;
    main.update(&model, .confirm_git_worktree_create, &fx);
    try testing.expectEqual(@as(u64, 0), model.git_worktree_add_key);
    try testing.expectEqual(@as(u64, 0), model.git_worktree_base_key);
    ...
    try testing.expectEqual(@as(u64, 0), model.git_worktree_base_key);
    const created = findGitWorktreeAddSpawnKey(&fx, model.git_worktree_add_key) orelse return error.MissingGitWorktreeAddSpawn;
    try expectGitWorktreeAddArgv(created, project, home, "feat-new", "main");
    try testing.expect(model.git_worktree_create_active);
    try testing.expectEqualStrings(project, model.selectedProjectPath());

    const add_key = model.git_worktree_add_key;
    main.update(&model, .confirm_git_worktree_create, &fx);
    ...
    main.update(&model, .{ .project_path_edit = .clear }, &fx);
    main.update(&model, .{ .project_path_edit = .{ .insert_text = project } }, &fx);
    const reset_branch = findGitBranchSpawnKey(&fx, model.git_branch_key) orelse return error.MissingGitBranchReset;
    try fx.feedLine(reset_branch.key, "main\n");
    drainEffects(&model, &fx);
    try testing.expectEqualStrings(project, model.selectedProjectPath());
...
fn findGitRevParseSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_branch.isGitRevParseArgv(spawn.argv)) return spawn;
    }
    return null;
}
...
fn findPendingGitCommitCachedQuiet(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and git_commit.isGitCommitCachedQuietArgv(spawn.argv)) return spawn;
    }
    return null;
}

fn findPendingGitCommitCmd(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
...
fn findGitReviewHunkSpawnKey(fx: *Effects, key: u64) ?@TypeOf(fx.pendingSpawnAt(0).?) {
    var i: usize = 0;
    while (fx.pendingSpawnAt(i)) |spawn| : (i += 1) {
        if (spawn.key == key and review_diff.isGitReviewHunkArgv(spawn.argv)) return spawn;
    }
    return null;
}
...
test "Environment Compare closes the dropdown and opens a Review file-list card" {
    ...
    const uncommitted_btn = try expectButtonMsg(tree, "Uncommitted", .set_review_diff_source_uncommitted);
    ...
    try testing.expectEqualStrings(review_diff.git_head, hunk.argv[7]);
    try testing.expectEqualStrings("--", hunk.argv[8]);
    try testing.expectEqualStrings("src/a.zig", hunk.argv[9]);
    try testing.expect(std.mem.indexOf(u8, hunk.argv[2], "src/a.zig") == null);
    try fx.feedLine(hunk.key, "diff --git a/src/a.zig b/src/a.zig\n+hello\n");
    drainEffects(&model, &fx);
...
test "Review source row switches Uncommitted and re-probes name-status plus untracked" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/review-source-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("review source ui", .fx);
    model.selected = id;
    main.update(&model, .{ .project_path_edit = .{ .insert_text = project } }, &fx);
    ...
    tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .text, review_diff.empty_status);
    try testing.expect(findByText(tree.root, .text, "M tracked.zig") == null);
...
test "Review source row switches Staged and re-probes --cached name-status" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/review-staged-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("review staged ui", .fx);
    model.selected = id;
    main.update(&model, .{ .project_path_edit = .{ .insert_text = project } }, &fx);
    ...
    const staged_off = try expectButtonMsg(tree, "Staged", .set_review_diff_source_staged);
    try testing.expect(!staged_off.state.selected);
    main.update(&model, tree.msgForPointer(staged_off.id, .up).?, &fx);
    try testing.expect(model.review_diff_active);
    try testing.expectEqual(review_diff.Source.staged, model.review_diff_source);
    try testing.expect(model.review_diff_source_staged());
    try testing.expect(!model.review_diff_source_branch());
    try testing.expect(!model.review_diff_source_uncommitted());
    try testing.expect(!model.review_diff_source_committed());
    try testing.expect(model.review_diff_key != initial_key);
    try testing.expectEqualStrings(review_diff.comparing_status, model.review_diff_status());
    try testing.expectEqual(@as(u32, 0), model.review_diff_file_count);
    try testing.expect(findGitReviewDiffSpawnKey(&fx, initial_key) == null);

    const staged_spawn = findGitReviewDiffSpawnKey(&fx, model.review_diff_key) orelse return error.MissingReviewDiffStaged;
    try testing.expect(review_diff.isGitReviewDiffArgv(staged_spawn.argv));
    ...
    tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .text, review_diff.empty_status);
    try testing.expect(findByText(tree.root, .text, "A staged.zig") == null);
    const branch_on = try expectButtonMsg(tree, "Branch", .set_review_diff_source_branch);
    try testing.expect(branch_on.state.selected);

    main.update(&model, .set_review_diff_source_staged, &fx);
    const fail_spawn = findGitReviewDiffSpawnKey(&fx, model.review_diff_key) orelse return error.MissingReviewDiffStagedFail;
    try testing.expectEqualStrings("--cached", fail_spawn.argv[8]);
    try fx.feedExit(fail_spawn.key, 128);
    drainEffects(&model, &fx);
    try testing.expectEqualStrings(review_diff.failed_status, model.review_diff_status());
    tree = try buildTree(arena, &model);
...
test "Review source row switches Committed and re-probes origin/HEAD...HEAD name-status" {
    ...
    defer tmp.cleanup();
    var project_buf: [256]u8 = undefined;
    const project = try std.fmt.bufPrint(&project_buf, ".zig-cache/tmp/{s}/review-committed-ui", .{tmp.sub_path[0..]});
    try std.Io.Dir.cwd().createDirPath(testing.io, project);
    ...
    var model = Model{};
    model.store_io = testing.io;
    const id = model.addSession("review committed ui", .fx);
    model.selected = id;
    main.update(&model, .{ .project_path_edit = .{ .insert_text = project } }, &fx);
    ...
    const committed_off = try expectButtonMsg(tree, "Committed", .set_review_diff_source_committed);
    try testing.expect(!committed_off.state.selected);
    main.update(&model, tree.msgForPointer(committed_off.id, .up).?, &fx);
    try testing.expect(model.review_diff_active);
    try testing.expectEqual(review_diff.Source.committed, model.review_diff_source);
    try testing.expect(model.review_diff_source_committed());
    try testing.expect(!model.review_diff_source_branch());
    try testing.expect(!model.review_diff_source_uncommitted());
    try testing.expect(!model.review_diff_source_staged());
    ...
    const committed_spawn = findGitReviewDiffSpawnKey(&fx, model.review_diff_key) orelse return error.MissingReviewDiffCommitted;
    try testing.expect(review_diff.isGitReviewDiffArgv(committed_spawn.argv));
    try testing.expectEqual(review_diff.argv_len, committed_spawn.argv.len);
    try testing.expectEqualStrings(review_diff.git_committed_range, committed_spawn.argv[8]);
    ...
    tree = try buildTree(arena, &model);
    _ = try expectByText(tree.root, .text, review_diff.empty_status);
    try testing.expect(findByText(tree.root, .text, "M committed.zig") == null);
    const branch_on = try expectButtonMsg(tree, "Branch", .set_review_diff_source_branch);
    try testing.expect(branch_on.state.selected);

    main.update(&model, .set_review_diff_source_committed, &fx);
    const fail_spawn = findGitReviewDiffSpawnKey(&fx, model.review_diff_key) orelse return error.MissingReviewDiffCommittedFail;
