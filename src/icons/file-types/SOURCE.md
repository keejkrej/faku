...
file icons (45 SVGs: first-cut coding-agent languages plus a
second-cut language/data expansion). That project is MIT; see its
...
`fill="url(#…)"`) and `graphql` (`transform=` on connecting bars)
are omitted — Native rejects those paints / ignores transforms.
