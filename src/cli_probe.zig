...
//! … opencode2=610, deepseek=611. fx (enum 0) is unused on this band. Distinct from
...
test "probeKey is per-id and skips fx_probe_key / ask / daemon" {
    ...
    try std.testing.expectEqual(@as(u64, 611), probeKey(.deepseek));
    ...
    try std.testing.expect(probeKey(.deepseek) != fx_probe.fx_probe_key);
    ...
    try std.testing.expectEqual(protocol.ProviderId.deepseek, fromProbeKey(611).?);
    ...
    try std.testing.expect(fromProbeKey(612) == null);
    try std.testing.expectEqual(@as(usize, 11), nonFxCount());
}
...
test "isCliProbeArgv matches PATH defaultBinary --help only" {
    ...
    try std.testing.expect(isCliProbeArgv(&.{ "dsh", "--help" }, .deepseek));
    ...
    try std.testing.expect(isAnyCliProbeArgv(&.{ "dsh", "--help" }));
