...
pub fn writeRefreshBackgroundWorkStdin(buf: []u8, args: RefreshBackgroundWorkStdin) WriteError![]const u8 {
    ...
    cur.pos += hello.len;
    try cur.write("\n");
    const refresh = try protocol.writeRefreshBackgroundWork(
        cur.remaining(),
        args.request_id,
        args.session_id,
        args.runtime_id,
...
pub fn run(io: std.Io, address: []const u8, outbound: []const u8, stdout: *std.Io.Writer) !void {
    ...
    const wait_for_background = outboundWaitsForBackgroundWork(outbound);
    ...
    while (true) {
        ...
        if (wait_for_load or wait_for_hydrate or wait_for_background) {
...
test "writeRefreshBackgroundWorkStdin emits hello and bare refreshBackgroundWork" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeRefreshBackgroundWorkStdin(&buf, .{
        .token = "secret",
        .session_id = "00000000-0000-0000-0000-000000000007",
        .runtime_id = "00000000-0000-0000-0000-000000000003",
    });
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"sessionId\":\"00000000-0000-0000-0000-000000000007\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"00000000-0000-0000-0000-000000000003\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"refreshBackgroundWork\"}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"prompt\"") == null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"loadUsageHistory\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"workspace\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ BACKGROUND_WORK_REQUEST_ID) != null);
    try std.testing.expect(!outboundWaitsForTurn(stdin));
    try std.testing.expect(!outboundWaitsForLoadResponse(stdin));
    try std.testing.expect(!outboundWaitsForHydrateResponse(stdin));
...
test "writeWorkspaceStdin emits hello and workspace hasRef with snake_case git_ref" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
        .operation = .{
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"" ++ protocol.NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ WORKSPACE_REQUEST_ID) != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"hasRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-0\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"git_ref\":\"refs/faku/session-7-turn-0\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"deleteSessionRefs\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"copySessionRefs\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"captureTurnStart\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"captureTurn\"") == null);
...
test "writeWorkspaceStdin emits hello and workspace captureRef with snake_case git_ref" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
        .operation = .{
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"" ++ protocol.NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ WORKSPACE_REQUEST_ID) != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"captureRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"hasRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"deleteSessionRefs\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"copySessionRefs\"") == null);
...
test "writeWorkspaceStdin emits hello and workspace restoreRef with snake_case git_ref" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
        .operation = .{
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"" ++ protocol.NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ WORKSPACE_REQUEST_ID) != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"restoreRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"captureRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"hasRef\"") == null);
...
test "writeWorkspaceStdin emits hello and workspace deleteRef with snake_case git_ref" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
        .operation = .{
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"" ++ protocol.NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ WORKSPACE_REQUEST_ID) != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"deleteRef\",\"cwd\":\"/tmp/faku\",\"git_ref\":\"refs/faku/session-7-turn-start-1\"}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"git_ref\":\"refs/faku/session-7-turn-start-1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"gitRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"restoreRef\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"captureRef\"") == null);
...
test "writeWorkspaceStdin emits hello and workspace createProjectlessWorkspace with prompt null vs string" {
    var buf: [1024]u8 = undefined;
    const stdin = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
    ...
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"runtimeId\":\"" ++ protocol.NIL_UUID ++ "\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"requestId\":\"" ++ WORKSPACE_REQUEST_ID) != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"command\":{\"type\":\"workspace\",\"operation\":{\"type\":\"createProjectlessWorkspace\",\"prompt\":null}}") != null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"discoverSlashCommands\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"prompt\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, stdin, "\"type\":\"attachSession\"") == null);
    ...
    const with_prompt = try writeWorkspaceStdin(&buf, .{
        .token = "secret",
        .operation = .{ .create_projectless_workspace = .{ .prompt = "Fix the login" } },
    });
    try std.testing.expect(std.mem.indexOf(u8, with_prompt, "\"prompt\":\"Fix the login\"") != null);
