...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  ...
  label reuses `i18n.ComposerProjectChrome.reveal_folder` via a distinct Model
  getter `skill_reveal_label`). Not Open in editor, not a daemon
  ...
  via Model `skill_path_copied_status`); label reuses
  `i18n.ComposerProjectChrome.copy_path` via a distinct Model getter
  `skill_copy_path_label`). Not Reveal, not Open in editor, not a
