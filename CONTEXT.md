...
`loadSkills`, `setSkillsEnabled`, `refreshBackgroundWork`, `stopBackgroundWork`. Start defaults to provider
...
Native FS API, not `sessions.json`. In-flight rename or
`setSkillsEnabled` fail closed. Refresh on open / Refresh, after a
successful rename, and after an ok `setSkillsEnabled` ack. When
...
untitled titles still use the original draft. Still not
`trashSkills` (no trash UI this cut). Enable / Disable chip and Disabled badge follow the resolved
...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  `SKILL.md` ↔ `SKILL.md.disabled` rename. `trashSkills` still not
  this cut (no trash UI).
