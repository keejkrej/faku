...
`unavailable` (daemon `errors[]` text stays English data this cut), plus a tiny scan-summary footer when `records` /
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  `pricing` is `unavailable` (daemon `errors[]` text stays English data this cut) plus a tiny scan-summary footer
