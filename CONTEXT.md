...
lists up to ~16 `projects` rows (path basename) with first-cut
relative bars (share vs the max project in the window for Cost →
`costUsd` or Tokens → `totalTokens`; Native `<progress>`; projects
with 0 stay text-only) and the same Cost | Tokens chip (flip
recomputes from the cached projects snapshot, no re-fetch). A same-shape
...
bars ship on Projects with that same chip; still not Waku's
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  `<progress>`; first-cut Projects bars relative to the max project
  in the window for that same chip, Native `<progress>`, chip flip
  does not re-fetch). Still not Waku's GPUI /
