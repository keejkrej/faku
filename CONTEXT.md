...
lines; Monthly lists up to ~12 `months` rows; Projects
...
"Connect a daemon for usage history" or stays empty. Share bars ship
on Daily; not a layered chart, not hover canvas, not quality /
rate-table fetch.
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  Not a T3 cost chart, not quality panel, not rate-table download, not
