...
hides the start page. Reload, Open in browser, Back, and Forward stay
...
`reload_token` on the next update tick (not Waku cache-clear; Native
`WebViewPane` documents only `url` + `reload_token`), Cmd/Ctrl-L focuses the address
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  (Faku-side blank-hop + `reload_token`; not Waku cache-clear; Native
  `web_panes` has no hard-reload flag), Cmd/Ctrl-L address field,
