...
auto-domain; empty `daily[]` hides the chart; still not Waku GPUI / T3 canvas
polish: smoothing, 12% fill opacity, stroke width, paint-order by period total) plus first-cut nested Claude/Codex Native
...
peak is > 0, else Native auto-domain; empty visible set / empty
`projects[]` hides the chart; x-labels are path basename in painted
row order; still not Waku GPUI / T3 canvas polish: smoothing, 12% fill
opacity, stroke width, paint-order by period total) plus first-cut
...
from zero, not stacked; still not Waku GPUI / T3 canvas polish: smoothing, 12%
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  sample when that peak is > 0, else Native auto-domain; still not Waku
  ...
  peak is > 0, else Native auto-domain; empty visible set / empty
  `projects[]` hides the chart; still not Waku GPUI / T3 canvas polish)
