...
`max`). Other palette commands and Build/Plan stay English.
...
- Full-app i18n catalogs / rust_i18n-style YAML, Native locale /
  ...
  `auto` / `none` / `minimal` / `low` / `medium` / `high` / `xhigh` /
  `max`). Other palette command labels and Build/Plan stay English.
