...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  ...
  path; label reuses `i18n.ComposerProjectChrome.copy_path` via a
  distinct Model getter `skill_copy_path_label`). Not Reveal, not Open
  in editor, not a daemon method for copy. This cut ships Settings
  ...
  Remaining Skills leftovers: path_copied toast, section headers,
  richer empty title/description.
  In-flight
