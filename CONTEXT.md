...
skills only (disabled stay hidden from insert; Send still ships the
composer text as-is; fx loads the skill). Enable / Disable chip and
Disabled badge follow the resolved locale this cut (same
...
empty reuses the same hint). Not SKILL.md body stuffing. Skill rows
