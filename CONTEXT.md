...
Unix packed `find` after `/bin/sh -c` chdir; Windows
`powershell.exe -NoProfile -Command {…} -Args <project_path>`
(`$args[0]`; same skip names / depth 8 / cap 64; does **not** prune
`.*` because skills live under `.cursor/skills` / `.agents/skills`;
relative `/` paths). Name is YAML `name:` when present, else the parent
...
no address keep today's local `find` / powershell walk. Composer `$name` insert ships from this scan for **enabled**
...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  unknown-command / parse / sidecar / empty / Native 4 KiB overflow /
  no address keep today's local `find` / powershell walk).
