...
hides the start page. Reload, Hard Reload, Open in browser, Back, and Forward stay
...
live turn). Sidebar Back/Forward stay session history.
Cmd/Ctrl-Alt-I DevTools, loading Stop,
and `page_title` stay out (Native `web_panes` has no title / stop /
hard-reload callbacks).
First-cut
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  live turn). DevTools / loading Stop / `page_title`
  stay out (Native `web_panes` has no title / stop / hard-reload
  callbacks).
  ...
  loading Stop / `page_title` still out).
