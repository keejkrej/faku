...
`SKILL.md` and `SKILL.md.disabled` files from
...
First-cut Enable/Disable is a Faku-side one-shot `mv --` rename in
that skill directory (`SKILL.md` ↔ `SKILL.md.disabled`; spawn-key
band 580+, distinct from the scan band 530+). Not a Native FS API,
not daemon SkillsCatalog / WorkspaceOperation, not `sessions.json`.
Refresh on open / Refresh, and after a successful rename.
Composer `$name` insert ships from this scan for **enabled** skills
only (disabled stay hidden from insert; Send still ships the composer
text as-is; fx loads the skill). Enable / Disable chip and Disabled
badge follow the resolved locale this cut (same
...
empty reuses the same hint). Not SKILL.md body stuffing. Windows
stays empty this cut. Skill rows stay runtime-only, not a live watch.
