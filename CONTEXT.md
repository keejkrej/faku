...
per-MTok hint on Model rows when lookup hits). Still not Waku's GPUI /
T3 layered / stacked canvas chart, not hover canvas, not a local
transcript scan.
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  lookup hit). Still not Waku's GPUI /
  T3 layered / stacked canvas chart, not a local transcript scan. Local session
