...
Waku `page_title` / DevTools / UUID tabs / Hard Reload.
...
(`reload_token`; no-op otherwise), Cmd/Ctrl-L focuses the address
...
Cmd/Ctrl-Shift-R Hard Reload, Cmd/Ctrl-Alt-I DevTools, loading Stop,
and `page_title` stay out (Native `web_panes` undocumented;
no title / stop / hard-reload callbacks).
First-cut
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  `page_title` / surface UUID tabs / DevTools / Hard Reload).
  First-cut Browser-tab-gated chords when that tab is showing and
  settings are closed: Cmd/Ctrl-R reload, Cmd/Ctrl-L address field,
  ...
  live turn). Hard Reload / DevTools / loading Stop / `page_title`
  stay out (Native `web_panes` undocumented).
  ...
  and paints a first-cut Native start page (Waku-shaped; Hard Reload /
  DevTools / loading Stop / `page_title` still out).
