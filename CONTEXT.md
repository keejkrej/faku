...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  Recycle Bin). Two-click Delete → Confirm delete. First-cut Open in editor for the
  ...
  with no selection / empty / invalid path; label reuses `i18n.FilePreviewChrome.open_in_editor`
  via a distinct Model getter). Not an embedded editor, not `open_in_app`, not a daemon
