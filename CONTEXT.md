...
when Compare was active or Diff was showing), Background selected row, and Browser occupancy / histories / active restore from an in-memory
per-session stash (Waku `RightPanelSessionState::take_or_closed` on
switch / New Task / remove; missing key → closed + `DEFAULT_FILE_TREE_WIDTH` 184 + Background row 0 + default Browser slot 0 empty history; cap last 16 session ids with LRU; not
`sessions.json`; stale Background row ids that are gone from the selected session's registry clear to 0). Selected tab, `right_panel_open`, nested widths, and Browser extras still persist globally on
`sessions.json` extras for boot; the stash overrides them on session
switch. Background row and the per-session Browser stash are not written to `sessions.json`. Every restore resets Files preview find/replace (Waku `reset_file_search_for_session`: close the bar, clear matches, clear find + replace buffers). Diff snapshot bodies re-fetch; selection re-applies when the
...
lazy single spawn). Scrollback/status/live process state stay
runtime-only (restore re-spawns fresh shells). `ptySpawn` has no cwd field;
