...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  then clear arming and `refresh`). Unknown-command / parse /
  sidecar / Native 4 KiB overflow / no address keep a Faku-side
  **permanent** directory remove of that skill dir (not OS Trash /
  Recycle Bin). Two-click Delete → Confirm delete. First-cut Open SKILL.md for the
