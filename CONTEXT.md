...
non-fx probes only. First-cut per-row Enable/Disable persists
`disabled_providers` (wire names) on `sessions.json` extras (default
empty = all enabled). Disabled skips background plan-usage refresh
unless the selected session already uses that id; Apply / started
sessions on that provider still work. `providerEnabled` is
`!disabled` this cut — not AND PATH `--help` probe-installed. Not
...
rows when Days is selected with first-cut relative bars (share vs the max day in the window
for Cost → `costUsd` or Tokens → `totalTokens`; Native `<progress>`;
days with 0 stay text-only) plus first-cut nested Claude/Codex Native
...
first-cut relative bars (share vs the max Cost|Tokens among
...
a Daily Model | Days breakdown (default Model; Days keeps the
first-cut daily bar chart plus first-cut nested byProvider Claude/Codex
Native `<progress>` rows) ship on Daily; first-cut monthly bars
...
project search filter (peak-of-visible shares; nested bars only on
visible rows); Daily, Monthly, and Projects paint a first-cut
...
`scannedFiles` are present. Still not Waku's GPUI / T3 layered /
stacked canvas chart, not hover canvas, not LiteLLM rate-table fetch.
...
selectedProvider != provider` (first-cut `providerEnabled` is
`!disabled_providers`, not AND probe-installed); unset per-provider
...
is disabled. Runtime-only first-cut plan_usage map
(four slots: Claude / Codex / OpenCode / Grok; not a HashMap);
switching session/provider shows that slot immediately and does not
clear the others. fx / cursor / amp / pi / kimi stay context-only.
Still not a circular path-drawn gauge, not LiteLLM.
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  Tokens computed from totals; Days keeps first-cut Daily bars
  relative to the max day in the window for that metric, Native
  ...
  `pricing` is `unavailable`. Still not Waku's GPUI /
  T3 layered / stacked canvas chart, not LiteLLM rate-table download. Local session
...
- Composer usage meter ships as a first-cut Native `<progress>` under
  ...
  disabled selected id). First-cut `disabled_providers` persist +
  Settings → Providers Enable/Disable ships (`providerEnabled` is
  `!disabled`; leftover: Waku also AND probe-installed, and disabling
  does not move unstarted drafts / last_provider — Faku new sessions
  stay fx and drafts.json has no provider). First-cut
  ...
  not LiteLLM, not a T3 layered / stacked canvas chart.
