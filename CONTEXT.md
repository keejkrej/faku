...
placeholder, Settings Usage Projects search-field placeholder + a11y
...
Settings Skills library / details pane titles follow the resolved locale
...
grow details), still not Waku GPUI virtualized list quirks; distinct from
...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  ...
  Native text after the filter field; `disabled` is total cached
  disabled like Waku library header; numbers stay Latin; ` · ` stays).
