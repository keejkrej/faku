...
| **Settings Skills** | Settings page that scans project `SKILL.md` and `SKILL.md.disabled` files (Waku `DISABLED_SKILL_FILE`). First-cut enable/disable renames `SKILL.md` ↔ `SKILL.md.disabled` in that skill directory (Faku-side one-shot: Unix `/bin/sh -c` chdir packed `find` / `mv --`; Windows `powershell.exe -NoProfile -Command {…} -Args` walk + Move-Item with argv slots for project cwd / skill-dir / from / to; not a Native FS API, not daemon SkillsCatalog / WorkspaceOperation, not `sessions.json`). Live file wins when both exist in the same dir. Skill rows stay runtime-only. Settings page nav persists as `settings_page` on `sessions.json` extras (`general` / `appearance` / `providers` / `skills` / `usage` / `computer_use`; missing / unknown / empty → General; same bag as theme / usage chrome / `ui_font_size`). Composer `$name` insert lists **enabled** skills only (disabled stay hidden from insert; Waku tools also ignore `.disabled`). Composer `/` slash card merges enabled `skill_store` rows after ACP `available_commands` (ACP name wins; prefix filter applies to both; Commands button stays ACP-only). Skill-sourced insert writes `/name `. First-cut Send auto-prepends stripped `SKILL.md` bodies for `$name` and skill-matching `/name` tokens (enabled rows only; missing/unreadable omit that skill; untitled titles still use the original draft). Still not daemon SkillsCatalog. Enable / Disable chip and Disabled badge follow the resolved locale this cut (same `i18n.SkillsEnableChrome` strings; distinct from `i18n.ProvidersChrome` Enable / Disable; wire ids / on-press stay English `toggle_skill_enabled`). Empty-state Open a project / No skills found follow the resolved locale this cut (same `i18n.SkillsEmptyChrome` strings; distinct from `i18n.FilterChrome` / `i18n.RightPanelChrome`; composer `$` insert empty reuses the same hint). |
...
Native FS API, not daemon SkillsCatalog / WorkspaceOperation, not
`sessions.json`. Refresh on open / Refresh, and after a successful
rename. Composer `$name` insert ships from this scan for **enabled**
...
daemon SkillsCatalog / WorkspaceOperation. Enable / Disable chip and Disabled badge follow the resolved
