...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  ...
  This cut ships Settings Skills selected-detail chrome (`i18n.SkillsDetailChrome`; No description / Invoke / Location / Contents when a skill is selected; description / `/name` / path stay data; Contents value is supporting-file count · bytes (`i18n.SkillsFileCountChrome` + Latin B/KB/MB; `{skill_body}` stays a separate block); distinct from SkillsEmptyChrome / SkillsSelectChrome / SkillsCountChrome / SkillsSectionChrome; Updated when SKILL.md mtime is known (`i18n.SkillsUpdatedChrome`; EN Updated / zh-CN 更新 / ja 更新日時; relative just now / m / h / d from SKILL.md mtime unix seconds at loadBody; fail closed when mtime cannot be read; distinct from `i18n.SkillsDetailChrome`); Allowed tools when YAML `allowed-tools` is present (`i18n.SkillsAllowedToolsChrome`; EN Tools / zh-CN 工具 / ja ツール; plain / quoted scalar copied at loadBody; fail closed when missing/empty/unfenced; distinct from `i18n.SkillsDetailChrome` / `i18n.SkillsUpdatedChrome` / `i18n.SkillsFileCountChrome`); no duplicate grouping this cut; composer `$` insert unchanged).
  This cut ships Settings Skills selected-detail Updated (`i18n.SkillsUpdatedChrome`; Waku `skills.detail_updated` + relative just now / m / h / d when SKILL.md mtime is known; fail closed when mtime cannot be read; no duplicate grouping this cut).
  This cut ships Settings Skills selected-detail Contents file_count · bytes (`i18n.SkillsFileCountChrome`; Waku supporting-file count + Latin B/KB/MB; SKILL.md body stays a separate block; fail closed when the skill dir cannot be walked; no duplicate grouping this cut).
  This cut ships Settings Skills selected-detail Allowed tools (`i18n.SkillsAllowedToolsChrome`; Waku `skills.allowed_tools`; EN Tools / zh-CN 工具 / ja ツール; YAML `allowed-tools:` plain / quoted scalar copied at loadBody; fail closed when missing/empty/unfenced; no duplicate grouping this cut).
  This cut ships local user skill roots on that User section without a daemon (Waku `user_skill_locations` that exist; `$HOME/.fx/skills` included; not Claude plugin `installed_plugins.json` roots; not provider-labeled rows).
  In-flight
