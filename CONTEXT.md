...
First-cut theme preference, UI font size, plus language selector. System theme follows
...
UI font size chips sit after Theme and before Language: 11 / 12 / 13 /
14 / 15 / 16 / 18 / 20 (Waku FONT_SIZES). Default 14. Missing / unknown /
out-of-range loads as 14. Preference persists as `ui_font_size` on
`sessions.json` extras (JSON number; same bag as theme / language).
`designTokens` applies documented `DesignTokens.themeWithOverrides`
typography rungs (body / label / title / button / heading / display)
scaled from house pack defaults so body tracks the selected size
(default 14 ≈ current look). `.pixel_snap = .{ .geometry = false }`
and Geist/house color scheme / contrast / reduce_motion stay.
Chip labels stay Latin digits in every locale. Title + description
follow `i18n.AppearanceFontChrome`. Code font size stays leftover
(Native typography is one rung ladder + mono face flag, not a
separate code size API).

Language is System / English / 简体中文 / 日本語 chips under UI font size.
...
/ UI font size / Language, theme chips, OS caption), the sidebar Settings gear a11y
...
- Code font size (Waku Appearance has a separate code size; Native
  typography is one rung ladder + `mono_font_id` face flag, not a
  separate mono size API). UI font size ships this cut (Waku
  FONT_SIZES; default 14; `sessions.json` `ui_font_size`; documented
  `themeWithOverrides` typography rungs)
