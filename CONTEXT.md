...
skills only (disabled stay hidden from insert). First-cut Send
auto-prepends stripped `SKILL.md` bodies for `$name` tokens of enabled
rows onto the `startPrompt` text (provider prompt and stored user
turn); untitled titles still use the original draft. Still not
daemon SkillsCatalog / WorkspaceOperation, not ACP `/name` slash
rows. Enable / Disable chip and Disabled badge follow the resolved
