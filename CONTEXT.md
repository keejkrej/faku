...
today's one occupied slot 0 + `browser_url` draft). Occupied Terminal
...
today's lazy single spawn). Full history rings /
back / forward / `reload_token` stay runtime-only (typing the address bar
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  URL, empty history keeps `1`..`4`; occupied slot URLs persist; not Waku
