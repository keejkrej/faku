...
empty history uses home `https://example.com` → lock). Secure follows
`currentUrl` / history tip, not the in-progress draft. Native has no
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  not the draft; Native has no text-field icon helper).
