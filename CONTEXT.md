...
`last_daemon_address` is set. Ok payload `planUsage` paints
...
miss keep a prior snapshot or a muted error. Runtime-only one-provider
cache; switching session/provider shows the matching snapshot or
empty/loading. fx / cursor / amp / pi / kimi stay context-only. Still
not a circular path-drawn gauge, not Waku's 300s/600s/stale/retry
cadence, not LiteLLM.
...
- Composer usage meter ships as a first-cut Native `<progress>` under
  ...
  Claude / Codex / OpenCode / Grok. Still not Waku's circular GPUI
  gauge, not the 300s/600s/stale/retry refresh cadence, not LiteLLM.
