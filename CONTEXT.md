...
project search filter (peak-of-visible shares); still not Waku's
GPUI / T3 layered chart, not hover canvas, not quality / rate-table
fetch.
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  chip flip does not re-fetch). Still not Waku's GPUI /
  T3 layered chart, not quality panel, not rate-table download, not
