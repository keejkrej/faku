...
switch. Background row and the per-session Browser / Terminal stash are not written to `sessions.json`. Every restore resets Files preview find/replace (Waku `reset_file_search_for_session`: close the bar, clear matches, clear find + replace buffers). Diff snapshot bodies re-fetch; selection re-applies when the
...
(Waku FileSearch is 20k). No GPUI match washes. Runtime-only; session restore resets the bar so matches do not point at another session's preview). Diff hosts Environment Compare / Review (Branch,
...
(JSON booleans; missing / unknown / null → false). Settings open
