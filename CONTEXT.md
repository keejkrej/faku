...
on non-fx stays demo (ACP has no image blocks this cut).
Unavailable cursor / opencode / grok still use the demo timer.
...
on `ProviderId.codex` when `providers.isAvailable` and there is no
image attach spawns one-shot `{binary} exec {prompt}` (argv slots;
empty stdin). Progress streams to stderr; the final agent message
prints to stdout. `reply_path` stays `.fx` with `fx_spawn_acp = false`
...
- Native Pi ACP / `--mode json` / RPC, image attach on Pi (print-mode
  ...
  Codex, Amp, and Pi stays demo. Kimi is not in `ProviderId`
