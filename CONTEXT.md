...
history (else session history). Sidebar Back/Forward stay session
history. Cmd/Ctrl-Shift-R Hard Reload, Cmd/Ctrl-Alt-I DevTools, Esc
Stop, and `page_title` stay out (Native `web_panes` undocumented;
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  session history). Hard Reload / DevTools / Stop / `page_title`
