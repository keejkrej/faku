...
insert / slash skill rows stay flat). Selected-detail No description / Invoke / Location / Contents follow the resolved locale this cut (same `i18n.SkillsDetailChrome` strings; English matches Waku `skills.no_description` / `detail_invoke` / `detail_location` / `detail_contents`; distinct from `i18n.SkillsEmptyChrome` / `i18n.SkillsSelectChrome` / `i18n.SkillsCountChrome` / `i18n.SkillsSectionChrome`; muted Native No description when YAML `description:` is empty; Invoke line is `/` + skill name; Location prefers the absolute install parent from `absSkillParent` else the cached store path; Contents then `{skill_body}` when `has_skill_body`; empty body stays empty; Updated when SKILL.md mtime is known (`i18n.SkillsUpdatedChrome`; EN Updated / zh-CN 更新 / ja 更新日時; relative just now / m / h / d from SKILL.md mtime unix seconds at loadBody; fail closed when mtime cannot be read; distinct from `i18n.SkillsDetailChrome`); no file_count / allowed_tools / duplicate grouping this cut; composer `$` insert unchanged). First-cut Open in editor for the selected skill ships (`open_skill_in_editor`; `open_editor.startOpenEditorAt` at the absolute `SKILL.md` / `SKILL.md.disabled` path from `joinProbeRelpath`; fail closed with no selection / empty / invalid path; label reuses `i18n.FilePreviewChrome.open_in_editor` via a distinct Model getter; zh-CN / ja follow locale like sibling Skills chrome). Not an embedded editor, not `open_in_app`, not a daemon method. First-cut Reveal for the selected skill ships (`reveal_skill`; `reveal_folder.startRevealPath` at the absolute `SKILL.md` / `SKILL.md.disabled` path; parent directory via Finder/xdg-open/explorer; fail closed with no selection / empty / unresolved path; label reuses `i18n.ComposerProjectChrome.reveal_folder` via a distinct Model getter `skill_reveal_label`; zh-CN / ja follow locale like sibling Skills chrome). Not Open in editor, not a daemon method. First-cut Copy path for the selected skill ships (`copy_skill_path`; Native `fx.writeClipboard` via `copy.copyText` / `copy_turn_key` at the absolute skill parent directory from `absSkillParent`; fail closed with no selection / empty / unresolved path (no clipboard write, no window_status); successful write sets window_status Path copied (`i18n.SkillsPathCopiedChrome.path_copied` via Model `skill_path_copied_status`); label reuses `i18n.ComposerProjectChrome.copy_path` via a distinct Model getter `skill_copy_path_label`; zh-CN / ja follow locale like sibling Skills chrome). Not Reveal, not Open in editor, not a daemon method. Skill rows
...
- First-cut daemon `LoadSkills` / `SkillsCatalog` ships as a
  ...
  **permanent** directory remove of that skill dir (not OS Trash /
  ...
  This cut ships Settings Skills selected-detail chrome (`i18n.SkillsDetailChrome`; No description / Invoke / Location / Contents when a skill is selected; description / `/name` / path / `{skill_body}` stay data; distinct from SkillsEmptyChrome / SkillsSelectChrome / SkillsCountChrome / SkillsSectionChrome; Updated when SKILL.md mtime is known (`i18n.SkillsUpdatedChrome`; EN Updated / zh-CN 更新 / ja 更新日時; relative just now / m / h / d from SKILL.md mtime unix seconds at loadBody; fail closed when mtime cannot be read; distinct from `i18n.SkillsDetailChrome`); no file_count / allowed_tools / duplicate grouping this cut; composer `$` insert unchanged).
  This cut ships Settings Skills selected-detail Updated (`i18n.SkillsUpdatedChrome`; Waku `skills.detail_updated` + relative just now / m / h / d when SKILL.md mtime is known; fail closed when mtime cannot be read; no file_count / allowed_tools / duplicate grouping this cut).
