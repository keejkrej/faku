...
folder` (data, not chrome). Palette command Collapse all folders
follows the resolved locale this cut (same
`i18n.Sidebar.collapse_all_folders` as the sidebar button). Composer
...
`build` / `plan`). Other palette commands stay English.
...
- Full-app i18n catalogs / rust_i18n-style YAML, Native locale /
  ...
  chrome). Palette command Collapse all folders follows the
  resolved locale this cut (same `i18n.Sidebar.collapse_all_folders`
  as the sidebar button). Composer and Settings General Ask / Auto /
  ...
  `max`). First-cut composer interaction chip and Settings General
  Build / Plan follow the resolved locale this cut (same
  `i18n.Interaction` strings; wire ids stay `build` / `plan`). Other
  palette command labels stay English.
