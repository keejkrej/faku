...
register as `app:lock` / `app:globe`). Still not Waku DevTools / UUID
tabs / Hard Reload.
First-cut
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  occupied slot URLs persist; not Waku surface UUID tabs / DevTools).
