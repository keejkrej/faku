...
register as `app:lock` / `app:globe`). Session chips are a first-cut
host / truncated `display_url` of that same committed URL (Waku
`tab_label` without `page_title`; Native `WebViewPane` has no title
callback). Empty history keeps occupancy-order `1`..`4`. Still not
Waku `page_title` / DevTools / UUID tabs.
...
Cmd/Ctrl-Alt-I DevTools and `page_title` stay out (Native `web_panes`
has no title / DevTools callbacks).
First-cut
multi-session inside the Browser tab (cap 4, chips + **New** + **Close**;
Close keeps at least one session; chips are display_url/host first-cut,
not Waku `page_title` / surface UUID tabs / DevTools).
...
- Embedded Browser first-cut (right-panel Browser is a Native canvas
  ...
  **Open in browser** stays OS-open via `open` / `xdg-open` / Windows
  ...
  occupied chips show host or truncated `display_url` from the committed
  URL, empty history keeps `1`..`4`; occupied slot URLs and history rings
  persist; `reload_token` stays runtime-only; not Waku
  `page_title` / surface UUID tabs / DevTools).
  ...
  callback). DevTools / `page_title`
  stay out (Native `web_panes` has no title / DevTools callbacks).
  ...
  and paints a first-cut Native start page (Waku-shaped; DevTools /
  `page_title` still out).
