...
visible rows); Daily also paints a first-cut five-tile Native
metric strip (`totalTokens` plus `totals.cachedInput` /
`uncachedInput` / `output` / `cacheCreation` / `reasoning` and
`quality.cacheSavingsUsd`; zeros still paint; Daily only) and a
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  chip flip does not re-fetch); first-cut Daily five-tile Native
  metric strip from `totalTokens` / `totals` / `quality.cacheSavingsUsd`
  (processed tokens / cached input / uncached input / output / cache
  savings; zeros still paint; Daily only this cut); first-cut Daily Cost quality panel from
