...
rows when Days is selected with a first-cut Native `<chart>` of that
window (Cost → `costUsd` or Tokens → `totalTokens` as oldest-first
`[]const f32`; empty `daily[]` hides the chart; still not Waku's
GPUI / T3 layered / stacked canvas) plus first-cut nested Claude/Codex Native
...
first-cut Native `<chart>` plus first-cut nested byProvider Claude/Codex
Native `<progress>` rows; still not T3 layered/stacked GPUI) ship on Daily; first-cut monthly bars
...
Native `<chart>` ships (not Waku's GPUI / T3 layered / stacked canvas,
...
- Usage history from daemon `LoadUsageHistory` ships as a first-cut
  ...
  Tokens computed from totals; Days keeps a first-cut Native
  `<chart>` of oldest-first Cost|Tokens day samples (still not T3
  layered / stacked GPUI), plus first-cut nested Claude/Codex Native `<progress>`
  ...
  lookup hit). First-cut Daily Days Native `<chart>` ships; still not
  Waku's GPUI / T3 layered / stacked canvas, not a local transcript
  scan. Local session
