...
(cursor, opencode; Waku `launch_for` argv `["acp"]`, Faku binaries
`cursor-agent` / `opencode`) or grok (`agent stdio`, not a bare
...
`providers.isAvailable` and there is no image attach spawns one-shot
`{binary} -p --output-format text {prompt}` (argv slots; empty stdin).
`reply_path` stays `.fx` with `fx_spawn_acp = false` so stdout lines
use the existing non-ACP `handleFxLine` path. Project cwd reuses
`fx_ask_chdir_script`. Not ACP, not `claude acp`, not stream-json, not
permissions bypass, not acp-proxy. Image attach and unavailable
claude stay demo.
...
attach adds documented `--image {path}` (same argv-slot pattern as
`fx ask --image`; not path-in-prompt). Progress streams to stderr;
...
Send on `ProviderId.pi` when `providers.isAvailable` and there is no
image attach spawns one-shot `{binary} -p {prompt}` (argv slots;
empty stdin; `--print` is the long form). Print mode prints the
response and exits. `reply_path` stays `.fx` with
`fx_spawn_acp = false` so stdout lines use the existing non-ACP
`handleFxLine` path. Project cwd reuses `fx_ask_chdir_script`. Not
ACP, not acp-proxy, not `--mode json`, not RPC, not `-a` /
`--approve` / invented dangerously-* flags. Image attach and
unavailable Pi stay demo.
...
`opencode acp`), and grok `agent stdio` are spawned one-shot per
...
is Available (documented `--image` when a composer image is
attached), or one-shot `amp -x` / `--execute` when Amp is
Available, or one-shot `pi -p` / `--print` when Pi is Available).
...
- Native Pi ACP / `--mode json` / RPC, image attach on Pi (print-mode
  one-shot `pi -p {prompt}` ships). Claude print-mode one-shot
  (`claude -p --output-format text`) ships; Codex exec
  one-shot (`codex exec {prompt}`, documented `--image` when a
  composer image is attached) ships; Amp execute-mode one-shot
  (`amp -x {prompt}`) ships. Claude, Codex, Amp, and Pi are not ACP /
  stream-json / a long-lived SDK session. Image attach on Claude,
  Amp, and Pi stays demo. Kimi is not in `ProviderId`
...
- ACP image blocks on non-fx (cursor / opencode / grok image attach
  stays demo; Codex uses `codex exec --image`, not ACP)
